...
class M_commentreads10 extends Model
{
    protected $table = 'comment_reads10';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor10_comments')
            ->select('id')
            ->where('faktor10id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads10')
            ->select('comment_id')
