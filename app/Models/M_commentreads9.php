...
class M_commentreads9 extends Model
{
    protected $table = 'comment_reads9';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor9_comments')
            ->select('id')
            ->where('faktor9id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads9')
            ->select('comment_id')
