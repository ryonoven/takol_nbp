...
class M_nilairisikokredit extends Model
{
    protected $table = 'risikokredit';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikokredit.*, users.fullname')
            ->join('users', 'users.id = risikokredit.user_id', 'left')
            ->where([
                'risikokredit.faktor1id' => $faktorId,
                'risikokredit.kodebpr' => $kodebpr
            ])
            ->orderBy('risikokredit.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    {
        return $this->db->query('ALTER TABLE risikokredit AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikokredit AUTO_INCREMENT = ?', [$value]);
    }
