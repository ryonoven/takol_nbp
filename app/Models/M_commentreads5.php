...
class M_commentreads5 extends Model
{
    protected $table = 'comment_reads5';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor5_comments')
            ->select('id')
            ->where('faktor5id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads5')
            ->select('comment_id')
