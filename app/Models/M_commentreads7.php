...
class M_commentreads7 extends Model
{
    protected $table = 'comment_reads7';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor7_comments')
            ->select('id')
            ->where('faktor7id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads7')
            ->select('comment_id')
