...
class M_risikooperasional extends Model
{
    protected $table = 'risikooperasional';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikooperasional.*, users.fullname')
            ->join('users', 'users.id = risikooperasional.user_id', 'left')
            ->where([
                'risikooperasional.faktor1id' => $faktorId,
                'risikooperasional.kodebpr' => $kodebpr
            ])
            ->orderBy('risikooperasional.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 48) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [36, 41, 44, 45, 46, 47])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Operasional Inheren';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE risikooperasional AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikooperasional AUTO_INCREMENT = ?', [$value]);
    }
