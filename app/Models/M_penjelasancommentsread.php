...
class M_penjelasancommentsread extends Model
{
    protected $table = 'penjelasan_reads';
     ...
     * @return int
     */
    public function countUnreadCommentsForUserByFactor($Id, $kodebpr, $userId, $periodeId): int
    {
        // Get all comment IDs for the given factor, kodebpr, and periode
        $commentsInFactor = $this->db->table('penjelasan_comments')
            ->select('id')
            // ->where('id', $Id)
        ...
        $readCommentIds = $this->db->table('penjelasan_reads')
            ->select('comment_id')
