...
class M_commentreads4 extends Model
{
    protected $table = 'comment_reads4';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor4_comments')
            ->select('id')
            ->where('faktor4id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads4')
            ->select('comment_id')
