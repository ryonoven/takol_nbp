...
class M_tksprofilrisiko extends Model
{
    protected $table = 'tksprofilrisiko';
    ...
    {
        return $this->db->table('tksprofilrisikocomments')
            ->select('tksprofilrisikocomments.*, users.fullname')
            ->join('users', 'users.id = tksprofilrisikocomments.user_id', 'left')
            ->where('tksprofilrisikocomments.subkategori', $subkategori)
            ->where('tksprofilrisikocomments.faktor1id', $faktorId)
            ->where('tksprofilrisikocomments.kodebpr', $kodebpr)
            ->where('tksprofilrisikocomments.periode_id', $periodeId)
            ->orderBy('tksprofilrisikocomments.created_at', 'DESC')
            ->get()
            ->getResultArray();
    }
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('tksprofilrisiko.*, users.fullname')
            ->join('users', 'users.id = tksprofilrisiko.user_id', 'left')
            ->where([
                'tksprofilrisiko.faktor1id' => $faktorId,
                'tksprofilrisiko.kodebpr' => $kodebpr
            ])
            ->orderBy('tksprofilrisiko.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRatas($faktorId, $kodebpr, $periodeId)
    {
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 13) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [1, 6, 10, 11, 12])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Kredit Inheren';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE tksprofilrisiko AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE tksprofilrisiko AUTO_INCREMENT = ?', [$value]);
    }
