...
class M_likuiditasinheren extends Model
{
    protected $table = 'likuiditasinheren';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('likuiditasinheren.*, users.fullname')
            ->join('users', 'users.id = likuiditasinheren.user_id', 'left')
            ->where([
                'likuiditasinheren.faktor1id' => $faktorId,
                'likuiditasinheren.kodebpr' => $kodebpr
            ])
            ->orderBy('likuiditasinheren.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Likuiditas Inheren';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE likuiditasinheren AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE likuiditasinheren AUTO_INCREMENT = ?', [$value]);
    }
