...
class M_commentreads12 extends Model
{
    protected $table = 'comment_reads12';
    ...
    public function countUnreadCommentsForUserByFactor($faktorId, $kodebpr, $userId, $periodeId): int
    {
        ...
        $commentsInFactor = $this->db->table('faktor12_comments')
            ->select('id')
            ->where('faktor12id', $faktorId)
        ...
        $readCommentIds = $this->db->table('comment_reads12')
            ->select('comment_id')
