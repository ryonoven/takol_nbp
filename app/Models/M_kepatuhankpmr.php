...
class M_kepatuhankpmr extends Model
{
    protected $table = 'risikokepatuhan_kpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikokepatuhan_kpmr.*, users.fullname')
            ->join('users', 'users.id = risikokepatuhan_kpmr.user_id', 'left')
            ->where([
                'risikokepatuhan_kpmr.faktor1id' => $faktorId,
                'risikokepatuhan_kpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('risikokepatuhan_kpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Kepatuhan KPMR';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE risikokepatuhan_kpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikokepatuhan_kpmr AUTO_INCREMENT = ?', [$value]);
    }
