...
class M_stratejikinheren extends Model
{
    protected $table = 'stratejikinheren';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('stratejikinheren.*, users.fullname')
            ->join('users', 'users.id = stratejikinheren.user_id', 'left')
            ->where([
                'stratejikinheren.faktor1id' => $faktorId,
                'stratejikinheren.kodebpr' => $kodebpr
            ])
            ->orderBy('stratejikinheren.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 179) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [171, 172, 175, 178])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Stratejik Inheren';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE stratejikinheren AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE stratejikinheren AUTO_INCREMENT = ?', [$value]);
    }
