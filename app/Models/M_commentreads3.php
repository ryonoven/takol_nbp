...
class M_commentreads3 extends Model
{
    protected $table = 'comment_reads3';
     ...
     * @return int
     */
    public function countUnreadCommentsForUserByFactor($faktor3Id, $kodebpr, $userId, $periodeId): int
    {
        // Get all comment IDs for the given factor, kodebpr, and periode
        $commentsInFactor = $this->db->table('faktor3_comments')
            ->select('id')
        ...
        $readCommentIds = $this->db->table('comment_reads3')
            ->select('comment_id')
