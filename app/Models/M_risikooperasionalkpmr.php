...
class M_risikooperasionalkpmr extends Model
{
    protected $table = 'risikooperasional_kpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikooperasional_kpmr.*, users.fullname')
            ->join('users', 'users.id = risikooperasional_kpmr.user_id', 'left')
            ->where([
                'risikooperasional_kpmr.faktor1id' => $faktorId,
                'risikooperasional_kpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('risikooperasional_kpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Operasional KPMR';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE risikooperasional_kpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikooperasional_kpmr AUTO_INCREMENT = ?', [$value]);
    }
