...
class M_reputasikpmr extends Model
{
    protected $table = 'reputasikpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('reputasikpmr.*, users.fullname')
            ->join('users', 'users.id = reputasikpmr.user_id', 'left')
            ->where([
                'reputasikpmr.faktor1id' => $faktorId,
                'reputasikpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('reputasikpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 168) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [151, 158, 162, 165])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Reputasi KPMR';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE reputasikpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE reputasikpmr AUTO_INCREMENT = ?', [$value]);
    }
