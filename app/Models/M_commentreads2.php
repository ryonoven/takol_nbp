...
class M_commentreads2 extends Model
{
    protected $table = 'comment_reads2';
     ...
     * @return int
     */
    public function countUnreadCommentsForUserByFactor($faktor2Id, $kodebpr, $userId, $periodeId): int
    {
        // Get all comment IDs for the given factor, kodebpr, and periode
        $commentsInFactor = $this->db->table('faktor2_comments')
            ->select('id')
        ...
        $readCommentIds = $this->db->table('comment_reads2')
            ->select('comment_id')
