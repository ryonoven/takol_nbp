...
class M_likuiditaskpmr extends Model
{
    protected $table = 'likuiditaskpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('likuiditaskpmr.*, users.fullname')
            ->join('users', 'users.id = likuiditaskpmr.user_id', 'left')
            ->where([
                'likuiditaskpmr.faktor1id' => $faktorId,
                'likuiditaskpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('likuiditaskpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 135) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [118, 125, 129, 132])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Likuiditas KPMR';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE likuiditaskpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE likuiditaskpmr AUTO_INCREMENT = ?', [$value]);
    }
