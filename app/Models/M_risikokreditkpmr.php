...
class M_risikokreditkpmr extends Model
{
    protected $table = 'risikokredit_kpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikokredit_kpmr.*, users.fullname')
            ->join('users', 'users.id = risikokredit_kpmr.user_id', 'left')
            ->where([
                'risikokredit_kpmr.faktor1id' => $faktorId,
                'risikokredit_kpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('risikokredit_kpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Kredit KPMR';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE risikokredit_kpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikokredit_kpmr AUTO_INCREMENT = ?', [$value]);
    }
