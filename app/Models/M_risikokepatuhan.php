...
class M_risikokepatuhan extends Model
{
    protected $table = 'risikokepatuhan';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('risikokepatuhan.*, users.fullname')
            ->join('users', 'users.id = risikokepatuhan.user_id', 'left')
            ->where([
                'risikokepatuhan.faktor1id' => $faktorId,
                'risikokepatuhan.kodebpr' => $kodebpr
            ])
            ->orderBy('risikokepatuhan.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Hasil Penilaian Tingkat Risiko Kepatuhan Inheren';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE risikokepatuhan AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE risikokepatuhan AUTO_INCREMENT = ?', [$value]);
    }
