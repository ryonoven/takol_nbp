...
class M_stratejikkpmr extends Model
{
    protected $table = 'stratejikkpmr';
    ...
    public function getKomentarByFaktorIdAndKodebpr($faktorId, $kodebpr)
    {
        return $this->db->table($this->table)
            ->select('stratejikkpmr.*, users.fullname')
            ->join('users', 'users.id = stratejikkpmr.user_id', 'left')
            ->where([
                'stratejikkpmr.faktor1id' => $faktorId,
                'stratejikkpmr.kodebpr' => $kodebpr
            ])
            ->orderBy('stratejikkpmr.created_at', 'ASC')
            ->get()
            ->getResultArray();
    }
    ...
    public function hitungRataRata($faktorId, $kodebpr, $periodeId)
    {
        if ($faktorId == 199) {
            $result = $this->select('AVG(penilaiankredit) as rata_rata, COUNT(*) as total')
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->whereIn('faktor1id', [182, 189, 193, 196])
    ...
    public function insertOrUpdateRataRata($rataRata, $faktorId, $kodebpr, $periodeId, $keterangan = null)
    {
        ...
        if ($keterangan === null) {
            if ($existing && !empty($existing['keterangan'])) {
                ...
                $keterangan = 'Penilaian seluruh faktor';
            }
        }
    ...
    {
        return $this->db->query('ALTER TABLE stratejikkpmr AUTO_INCREMENT = 1');
    }

    public function setIncrement($value)
    {
        $value = (int) $value;
        return $this->db->query('ALTER TABLE stratejikkpmr AUTO_INCREMENT = ?', [$value]);
    }
