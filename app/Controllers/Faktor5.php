...
use App\Models\M_faktor5komentar;
use App\Models\M_nilaifaktor5;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads5;
use Myth\Auth\Config\Services as AuthServices;

class Faktor5 extends Controller
{
    protected $auth;
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor5komentar();
        $this->nilai5Model = new M_nilaifaktor5();
        $this->infobprModel = new M_infobpr();
        $this->commentReads5Model = new M_commentreads5();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor5id dari 1 hingga 5
        for ($faktor5Id = 1; $faktor5Id <= 6; $faktor5Id++) {
            $accdekomValue = $this->nilai5Model
        ...
        // Mengambil nilai accdekom untuk faktor5id 1 hingga 6
        $accdekomApproved = true;
        for ($faktor5Id = 1; $faktor5Id <= 5; $faktor5Id++) {
            $nilaiAccdekom = $this->nilai5Model
                ->where('faktor5id', $faktor5Id)
        ...
        $faktorData = $this->faktor5Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor5id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 6); // Membuat array dari 1 hingga 6
        foreach ($requiredFaktorIds as $faktor5Id) {
            $associatedNilai = $this->nilai5Model
                ->where('faktor5id', $faktor5Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor5Id) {
        //     // Ambil nilai untuk faktor5id
        //     $associatedNilai = $this->nilai5Model->where('faktor5id', $faktor5Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai5Model->hitungRataRata($faktor5Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai5Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai5Model->insertOrUpdateRataRata($rataRata, $faktor5Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor5Id' => $faktor5Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads5Model' => $this->commentReads5Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor5Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor5Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor5Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor5Id)) {
            // log_message('debug', 'Invalid AJAX or faktor5Id: ' . $faktor5Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai5Model->getNilaiByFaktorId($faktor5Id);

        // log_message('debug', 'Comments returned for faktor5Id ' . $faktor5Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor5Id = $this->request->getPost('faktor5_id');
                $data = [
                    'faktor5id' => $faktor5Id,
                ...
                $this->nilai5Model->tambahNilai($data, $faktor5Id, $kodebpr);

                // Calculate average value from faktor5id 1–8
                $rataRata = $this->nilai5Model->hitungRataRata($faktor5Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor5id 1 in nfaktor column (insert or update)
                $this->nilai5Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor5') . '?modal_nilai=' . $faktor5Id);
            }
        } else {
            return redirect()->to(base_url('faktor5'));
        }
    }
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor5') . '?modal_komentar=' . $this->request->getPost('faktor5_id'));
        }
    ...
    {
        $faktor5id = $this->request->getPost('faktor5id');
        $userId = service('authentication')->id();
        ...
        // Pastikan update berdasarkan faktor5id, kodebpr, dan periode_id
        if ($this->nilai5Model->ubahBerdasarkanFaktorId($data, $faktor5id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor5id 5
            $this->nilai5Model->where('faktor5id', 6)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    {
        $data = [
            'faktor5' => $this->faktor5Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor5Model
        ];

        echo view('faktor5/excel', $data);
    }
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor5Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor5Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor5_id
        $nilaiFaktor5 = $this->nilai5Model->find($idNilai);
        if (!$nilaiFaktor5) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor5 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai5Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai5Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai5Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor5 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai5Model
                ->where('faktor5id', 6)  // Only update where faktor5id = 6
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor5');
                return redirect()->back();
            }
            ...
            session()->setFlashdata('message', 'Faktor 5 berhasil di-unapprove.');
            return redirect()->back();
    ...
    public function accdekom()
    {
        ...
        if (!$faktor5id || !$kodebpr || !$periodeId) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap']);
        }

        // Ambil data faktor5 berdasarkan faktor5id, kodebpr, dan periode_id
        $nilaiFaktor5 = $this->nilai5Model
            ->where('faktor5id', $faktor5id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->first();
        ...
        $this->nilai5Model->where('faktor5id', 6)->update(null, $dataUpdateForFaktor6);
        ...
        if ($this->nilai5Model->update($nilaiFaktor5['id'], $dataUpdate)) {
            return redirect()->to('/faktor5')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor5')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function unapprovedekom()
    {
        ...
        // Update record dalam database
        if ($this->nilai5Model->update($nilaiFaktor5['id'], $dataUpdate)) {
            // Update accdekom untuk faktor5id 6
            $this->nilai5Model->where('faktor5id', 6)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->set(['accdekom' => 0])
            ...
            return redirect()->to('/faktor5')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor5')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor5id 1 to 5
        $allApproved = true;
        for ($faktor5Id = 1; $faktor5Id <= 5; $faktor5Id++) {
            $nilaiFaktor5 = $this->nilai5Model
                ->where('faktor5id', $faktor5Id)
            ...
            if (!$nilaiFaktor5 || $nilaiFaktor5['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor5Id = $this->request->getGet('faktor5_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor5Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads5Model->countUnreadCommentsForUserByFactor($faktor5Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads5Model->markAsRead($comment['id'], $userId);
            }
        }
