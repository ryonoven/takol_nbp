...
use App\Models\M_faktor3komentar;
use App\Models\M_nilaifaktor3;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads3;
...
class Faktor3 extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor3komentar();
        $this->nilai3Model = new M_nilaifaktor3();
        $this->infobprModel = new M_infobpr();
        $this->commentReads3Model = new M_commentreads3();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor3id dari 1 hingga 26
        for ($faktor3Id = 1; $faktor3Id <= 27; $faktor3Id++) {
            $accdekomValue = $this->nilai3Model
        ...
        // Mengambil nilai accdekom untuk faktor3id 1 hingga 27
        $accdekomApproved = true;
        for ($faktor3Id = 1; $faktor3Id <= 26; $faktor3Id++) {
            $nilaiAccdekom = $this->nilai3Model
                ->where('faktor3id', $faktor3Id)
        ...
        $faktorData = $this->faktor3Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor3id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 26); // Membuat array dari 1 hingga 27
        foreach ($requiredFaktorIds as $faktor3Id) {
            $associatedNilai = $this->nilai3Model
                ->where('faktor3id', $faktor3Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor3Id) {
        //     // Ambil nilai untuk faktor3id
        //     $associatedNilai = $this->nilai3Model->where('faktor3id', $faktor3Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai3Model->hitungRataRata($faktor3Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai3Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai3Model->insertOrUpdateRataRata($rataRata, $faktor3Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor3Id' => $faktor3Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads3Model' => $this->commentReads3Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor3Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor3Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor3Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor3Id)) {
            // log_message('debug', 'Invalid AJAX or faktor3Id: ' . $faktor3Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai3Model->getNilaiByFaktorId($faktor3Id);

        // log_message('debug', 'Comments returned for faktor3Id ' . $faktor3Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor3Id = $this->request->getPost('faktor3_id');
                $data = [
                    'faktor3id' => $faktor3Id,
                ...
                $this->nilai3Model->tambahNilai($data, $faktor3Id, $kodebpr);

                // Calculate average value from faktor3id 1–8
                $rataRata = $this->nilai3Model->hitungRataRata($faktor3Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor3id 1 in nfaktor column (insert or update)
                $this->nilai3Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor3') . '?modal_nilai=' . $faktor3Id);
            }
        } else {
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor3') . '?modal_komentar=' . $this->request->getPost('faktor3_id'));
        }
    ...
    {
        $faktor3id = $this->request->getPost('faktor3id');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai3Model->ubahBerdasarkanFaktorId($data, $faktor3id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor3id 26
            $this->nilai3Model->where('faktor3id', 27)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    public function ubahkesimpulan()
    {
        ...
        // Pastikan update berdasarkan faktor3id, kodebpr, dan periode_id
        if ($this->nilai3Model->ubahBerdasarkanFaktorId($data, $faktor3Id, $kodebpr, $periodeId)) {
            session()->setFlashdata('message', 'Data berhasil diubah');
        } else {
            session()->setFlashdata('err', 'Gagal mengubah data');
    ...
    {
        $data = [
            'faktor3' => $this->faktor3Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor3Model
        ];
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor3Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor3Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor3_id
        $nilaiFaktor3 = $this->nilai3Model->find($idNilai);
        if (!$nilaiFaktor3) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor3 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai3Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai3Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai3Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor3 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai3Model
                ->where('faktor3id', 26)  // Only update where faktor3id = 26
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor3 26');
                return redirect()->back();
            }
            ...
            session()->setFlashdata('message', 'Faktor 3 berhasil di-unapprove.');
            return redirect()->back();
    ...
    public function accdekom()
    {
        ...
        $this->nilai3Model->where('faktor3id', 27)->update(null, $dataUpdateForFaktor27);
        ...
        if ($this->nilai3Model->update($nilaiFaktor3['id'], $dataUpdate)) {
            return redirect()->to('/faktor3')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor3')->with('error', 'Gagal memperbarui data');
        }
    }
    ...
    public function unapprovedekom()
    {
        ...
        // Ambil data faktor3 berdasarkan faktor3id, kodebpr, dan periode_id
        $nilaiFaktor3 = $this->nilai3Model
            ->where('faktor3id', $faktor3id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
        ...
        if ($faktor3id == 27) {
            ...
            $this->nilai3Model->where('faktor3id', 12)->update(null, $dataUpdateForFaktor27);
        }
    ...
    public function unapprovedekom2()
    {
        ...
        // Update record dalam database
        if ($this->nilai3Model->update($nilaiFaktor3['id'], $dataUpdate)) {
            return redirect()->to('/faktor3')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor3')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor3id 1 to 26
        $allApproved = true;
        for ($faktor3Id = 1; $faktor3Id <= 26; $faktor3Id++) {
            $nilaiFaktor3 = $this->nilai3Model
                ->where('faktor3id', $faktor3Id)
            ...
            if (!$nilaiFaktor3 || $nilaiFaktor3['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor3Id = $this->request->getGet('faktor3_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor3Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads3Model->countUnreadCommentsForUserByFactor($faktor3Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads3Model->markAsRead($comment['id'], $userId);
            }
        }
