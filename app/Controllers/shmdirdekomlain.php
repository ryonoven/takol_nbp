...
class shmdirdekomlain extends Controller
{
    ...
    {
        if (!$this->shmdirdekomlainModel) {
            $this->shmdirdekomlainModel = new M_shmdirdekomlain();
        }
        return $this->shmdirdekomlainModel;
    }
    ...
    private function getIndexData($periodeId, $kodebpr)
    {
        ...
        $shmdirdekomlainData = $this->getSahamdirdekomModel()
    ...
    public function index()
    {
        ...
        $kodebpr = $this->userKodebpr; // ambil data user
        ...
        $accdekomData = $this->shmdirdekomlainModel
            ->select('accdekom, accdekom_by, accdekom_at')
            ->where('periode_id', $periodeId)
            ->where('kodebpr', $kodebpr)
            ->findAll();

        $accdirutData = $this->shmdirdekomlainModel
        ...
        $accdekomValues = $this->shmdirdekomlainModel
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
        ...
        $data = [
            ...
            'shmdirdekomlain' => $indexData['shmdirdekomlain'],
    ...
    private function validateSahamDir($data)
    {
        return $this->validate([
            ...
            'jenisperusahaan' => [
                'label' => 'Jenis Perusahaan Lain.',
                'rules' => 'required',
                'errors' => ['required' => '{field} tidak boleh kosong.']
            ],
            'perusahaan' => [
                'label' => 'Nama Bank / Perusahaan lain BPR',
                'rules' => 'required',
                'errors' => ['required' => '{field} tidak boleh kosong.']
            ],
            'persensaham' => [
    ...
    public function tambahsahamdir()
    {
        ...
        $specificData = [
            ...
            'jenisperusahaan' => $this->request->getPost('jenisperusahaan'),
            'perusahaan' => $this->request->getPost('perusahaan'),
            'persensaham' => $this->request->getPost('persensaham'),
            'kodebank' => $this->request->getPost('kodebank')
        ];
    ...
    public function Tambahkomentar()
    {
        ...
        return redirect()->to(base_url('Shmdirdekomlain') . '?modal_komentar=' . $this->request->getPost('id'));
    }
    ...
    public function hapus($id)
    {
        ...
        // Filter by kodebpr and periode_id before deletion
        $kodebpr = $this->userKodebpr;
        $periodeId = session('active_periode');
        ...
        // Deleting based on id, kodebpr, and periode_id
        $this->shmdirdekomlainModel->builder()
            ->where('id', $id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->delete();
    ...
    public function ubahdir()
    {
        ...
        if (empty($nama) || empty($nik) || empty($jenisperusahaan) || empty($perusahaan) || empty($persensaham) || empty($kodebank)) {
    ...
    public function ubahdekom()
    {
        ...
        if (empty($nama) || empty($nik) || empty($jenisperusahaan) || empty($perusahaan) || empty($persensaham)) {
            return redirect()->back()->with('error', 'Semua field harus diisi');
        }
...
// Dimana pembatalan komisaris akan membatalkan direktur juga
    ...
    public function exporttxtshmdirdekomlain()
    {
        ...
        $this->shmdirdekomlainModel = model('M_shmdirdekomlain');
        ...
        $data_shmdirdekomlain = $this->shmdirdekomlainModel->getDataByKodebprAndPeriode($kodebpr, $periodeId);
        ...
        $output = "";
        $output .= "H01|" . $kodejenis . "|" . $sandibpr . "|" . $exportDate . "-12-31|" . "LTBPRK|E0303|0|" . "\r\n";

        foreach ($data_shmdirdekomlain as $row) {
            $hasValidData = false;
            $nik = isset($row['nik']) && !$isEmpty($row['nik']) ? $row['nik'] : '';
            $nama = isset($row['nama']) && !$isEmpty($row['nama']) ? $row['nama'] : '';
            $perusahaan = isset($row['perusahaan']) && !$isEmpty($row['perusahaan']) ? $row['perusahaan'] : '';
            $persensaham = isset($row['persensaham']) && !$isEmpty($row['persensaham']) ? $row['persensaham'] : '';
            $kodebank = isset($row['kodebank']) && !$isEmpty($row['kodebank']) ? $row['kodebank'] : '';

            if ($nik !== '' || $nama !== '' || $perusahaan !== '' || $persensaham !== '' || $kodebank !== '') {
                $hasValidData = true;
            }

            // Hanya generate jika ada data valid
            if ($hasValidData) {
                $output .= "D01|" . "022010000000" . "|" . $nik . "|" . $nama . "|" . $kodebank . "|" . $perusahaan . "|" . $persensaham . "\r\n";
            }
        }
        ...
        $filename = "LTBPRK-E0303-R-A-" . $exportDate . "1231-" . $sandibpr . "-01.txt";
