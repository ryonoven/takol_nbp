...
class Faktor2 extends Controller
{
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            if (!$val) {
                ...
                if ($success) {
                    session()->setFlashdata('message', 'Komentar Dewan Komisaris berhasil ditambahkan');
    ...
    public function approve($idFaktor2)
    {
        if (!is_numeric($idFaktor2) || $idFaktor2 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
        }

        $faktor = $this->faktor2Model->find($idFaktor2);
        if (!$faktor) {
        ...
        if ($this->faktor2Model->save($dataUpdate)) {
    ...
    public function unapprove($idFaktor2)
    {
        if (!is_numeric($idFaktor2) || $idFaktor2 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
    ...
    public function approveSemua()
    {
        ...
        // Update all records
        if ($this->faktor2Model->builder()->update($dataUpdate)) {
