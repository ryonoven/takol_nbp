...
use App\Models\M_faktor2komentar;
use App\Models\M_nilaifaktor2;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads2;
use Myth\Auth\Config\Services as AuthServices;

class Faktor2 extends Controller
{
    protected $auth;
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor2komentar();
        $this->nilai2Model = new M_nilaifaktor2();
        $this->infobprModel = new M_infobpr();
        $this->commentReads2Model = new M_commentreads2();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor2id dari 1 hingga 29
        for ($faktor2Id = 1; $faktor2Id <= 29; $faktor2Id++) {
            $accdekomValue = $this->nilai2Model
        ...
        // Mengambil nilai accdekom untuk faktor2id 1 hingga 28
        $accdekomApproved = true;
        for ($faktor2Id = 1; $faktor2Id <= 28; $faktor2Id++) {
            $nilaiAccdekom = $this->nilai2Model
                ->where('faktor2id', $faktor2Id)
        ...
        $faktorData = $this->faktor2Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor2id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 28); // Membuat array dari 1 hingga 28
        foreach ($requiredFaktorIds as $faktor2Id) {
            $associatedNilai = $this->nilai2Model
                ->where('faktor2id', $faktor2Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor2Id) {
        //     // Ambil nilai untuk faktor2id
        //     $associatedNilai = $this->nilai2Model->where('faktor2id', $faktor2Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai2Model->hitungRataRata($faktor2Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai2Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai2Model->insertOrUpdateRataRata($rataRata, $faktor2Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor2Id' => $faktor2Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads2Model' => $this->commentReads2Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor2Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor2Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor2Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor2Id)) {
            // log_message('debug', 'Invalid AJAX or faktor2Id: ' . $faktor2Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai2Model->getNilaiByFaktorId($faktor2Id);

        // log_message('debug', 'Comments returned for faktor2Id ' . $faktor2Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor2Id = $this->request->getPost('faktor2_id');
                $data = [
                    'faktor2id' => $faktor2Id,
                ...
                $this->nilai2Model->tambahNilai($data, $faktor2Id, $kodebpr);

                // Calculate average value from faktor2id 1–8
                $rataRata = $this->nilai2Model->hitungRataRata($faktor2Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor2id 1 in nfaktor2 column (insert or update)
                $this->nilai2Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor2') . '?modal_nilai=' . $faktor2Id);
            }
        } else {
            return redirect()->to(base_url('faktor2'));
        }
    }
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor2') . '?modal_komentar=' . $this->request->getPost('faktor2_id'));
        }
    ...
    {
        $faktor2id = $this->request->getPost('faktor2id');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai2Model->ubahBerdasarkanFaktorId($data, $faktor2id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor2id 29
            $this->nilai2Model->where('faktor2id', 29)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    public function ubahkesimpulan()
    {
        ...
        // Pastikan update berdasarkan faktor2id, kodebpr, dan periode_id
        if ($this->nilai2Model->ubahBerdasarkanFaktorId($data, $faktor2Id, $kodebpr, $periodeId)) {
            session()->setFlashdata('message', 'Data berhasil diubah');
        } else {
            session()->setFlashdata('err', 'Gagal mengubah data');
    ...
    {
        $data = [
            'faktor2' => $this->faktor2Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor2Model
        ];

        echo view('faktor2/excel', $data);
    }
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor2Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor2Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor2_id
        $nilaiFaktor2 = $this->nilai2Model->find($idNilai);
        if (!$nilaiFaktor2) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor2 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai2Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai2Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai2Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor2 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai2Model
                ->where('faktor2id', 29)  // Only update where faktor2id = 29
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor2 29');
                return redirect()->back();
            }
            ...
            session()->setFlashdata('message', 'Faktor 29 berhasil di-unapprove.');
            return redirect()->back();
    ...
    public function accdekom()
    {
        ...
        if (!$faktor2id || !$kodebpr || !$periodeId) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap']);
        }

        // Ambil data faktor2 berdasarkan faktor2id, kodebpr, dan periode_id
        $nilaiFaktor2 = $this->nilai2Model
            ->where('faktor2id', $faktor2id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->first();
        ...
        $this->nilai2Model->where('faktor2id', 29)->update(null, $dataUpdateForFaktor29);
        ...
        if ($this->nilai2Model->update($nilaiFaktor2['id'], $dataUpdate)) {
            return redirect()->to('/faktor2')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor2')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function unapprovedekom()
    {
        ...
        // Update record dalam database
        if ($this->nilai2Model->update($nilaiFaktor2['id'], $dataUpdate)) {
            // Update accdekom untuk faktor2id 29
            $this->nilai2Model->where('faktor2id', 29)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->set(['accdekom' => 0])
            ...
            return redirect()->to('/faktor2')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor2')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor2id 1 to 29
        $allApproved = true;
        for ($faktor2Id = 1; $faktor2Id <= 29; $faktor2Id++) {
            $nilaiFaktor2 = $this->nilai2Model
                ->where('faktor2id', $faktor2Id)
            ...
            if (!$nilaiFaktor2 || $nilaiFaktor2['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor2Id = $this->request->getGet('faktor2_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor2Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads2Model->countUnreadCommentsForUserByFactor($faktor2Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads2Model->markAsRead($comment['id'], $userId);
            }
        }
