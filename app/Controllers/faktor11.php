...
use App\Models\M_faktor11komentar;
use App\Models\M_nilaifaktor11;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads11;
use Myth\Auth\Config\Services as AuthServices;

class Faktor11 extends Controller
{
    protected $auth;
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor11komentar();
        $this->nilai11Model = new M_nilaifaktor11();
        $this->infobprModel = new M_infobpr();
        $this->commentReads11Model = new M_commentreads11();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor11id dari 1 hingga 5
        for ($faktor11Id = 1; $faktor11Id <= 13; $faktor11Id++) {
            $accdekomValue = $this->nilai11Model
        ...
        // Mengambil nilai accdekom untuk faktor11id 1 hingga 5
        $accdekomApproved = true;
        for ($faktor11Id = 1; $faktor11Id <= 12; $faktor11Id++) {
            $nilaiAccdekom = $this->nilai11Model
                ->where('faktor11id', $faktor11Id)
        ...
        $faktorData = $this->faktor11Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor11id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 13); // Membuat array dari 1 hingga 13
        foreach ($requiredFaktorIds as $faktor11Id) {
            $associatedNilai = $this->nilai11Model
                ->where('faktor11id', $faktor11Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor11Id) {
        //     // Ambil nilai untuk faktor11id
        //     $associatedNilai = $this->nilai11Model->where('faktor11id', $faktor11Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai11Model->hitungRataRata($faktor11Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai11Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai11Model->insertOrUpdateRataRata($rataRata, $faktor11Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor11Id' => $faktor11Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads11Model' => $this->commentReads11Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    public function cekKomentarBaru()
    {
        ...
        $results = $this->komentarModel
            ->select('faktor11id, COUNT(*) as jumlah')
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor11Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor11Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor11Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor11Id)) {
            // log_message('debug', 'Invalid AJAX or faktor11Id: ' . $faktor11Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai11Model->getNilaiByFaktorId($faktor11Id);

        // log_message('debug', 'Comments returned for faktor11Id ' . $faktor11Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor11Id = $this->request->getPost('faktor11_id');
                $data = [
                ...
                $this->nilai11Model->tambahNilai($data, $faktor11Id, $kodebpr);

                // Calculate average value from faktor11id 1–8
                $rataRata = $this->nilai11Model->hitungRataRata($faktor11Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor11id 1 in nfaktor column (insert or update)
                $this->nilai11Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor11') . '?modal_nilai=' . $faktor11Id);
            }
        } else {
            return redirect()->to(base_url('faktor11'));
        }
    }
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor11') . '?modal_komentar=' . $this->request->getPost('faktor11_id'));
        }
    ...
    {
        $faktor11id = $this->request->getPost('faktor11id');
        $userId = service('authentication')->id();
        ...
        // Pastikan update berdasarkan faktor11id, kodebpr, dan periode_id
        if ($this->nilai11Model->ubahBerdasarkanFaktorId($data, $faktor11id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor11id 13
            $this->nilai11Model->where('faktor11id', 13)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    {
        $data = [
            'faktor11' => $this->faktor11Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor11Model
        ];

        echo view('faktor11/excel', $data);
    }
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor11Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor11Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor11_id
        $nilaiFaktor11 = $this->nilai11Model->find($idNilai);
        if (!$nilaiFaktor11) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor11 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai11Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai11Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai11Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor11 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai11Model
                ->where('faktor11id', value: 13)  // Only update where faktor11id = 13
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor11');
                return redirect()->back();
            }
            ...
            session()->setFlashdata('message', 'Faktor 11 berhasil di-unapprove.');
            return redirect()->back();
    ...
    public function accdekom()
    {
        ...
        if (!$faktor11id || !$kodebpr || !$periodeId) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap']);
        }

        // Ambil data faktor11 berdasarkan faktor11id, kodebpr, dan periode_id
        $nilaiFaktor11 = $this->nilai11Model
            ->where('faktor11id', $faktor11id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->first();
        ...
        $this->nilai11Model->where('faktor11id', 13)->update(null, $dataUpdateForFaktor11);
        ...
        if ($this->nilai11Model->update($nilaiFaktor11['id'], $dataUpdate)) {
            return redirect()->to('/faktor11')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor11')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function unapprovedekom()
    {
        ...
        // Update record dalam database
        if ($this->nilai11Model->update($nilaiFaktor11['id'], $dataUpdate)) {
            // Update accdekom untuk faktor11id 13
            $this->nilai11Model->where('faktor11id', 13)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->set(['accdekom' => 0])
            ...
            return redirect()->to('/faktor11')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor11')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor11id 1 to 5
        $allApproved = true;
        for ($faktor11Id = 1; $faktor11Id <= 5; $faktor11Id++) {
            $nilaiFaktor11 = $this->nilai11Model
                ->where('faktor11id', $faktor11Id)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
            ...
            if (!$nilaiFaktor11 || $nilaiFaktor11['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor11Id = $this->request->getGet('faktor11_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor11Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads11Model->countUnreadCommentsForUserByFactor($faktor11Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads11Model->markAsRead($comment['id'], $userId);
            }
        }
