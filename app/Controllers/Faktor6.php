...
use App\Models\M_faktor6komentar;
use App\Models\M_nilaifaktor6;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads6;
use Myth\Auth\Config\Services as AuthServices;

class Faktor6 extends Controller
{
    protected $auth;
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor6komentar();
        $this->nilai6Model = new M_nilaifaktor6();
        $this->infobprModel = new M_infobpr();
        $this->commentReads6Model = new M_commentreads6();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor6id dari 1 hingga 9
        for ($faktor6Id = 1; $faktor6Id <= 10; $faktor6Id++) {
            $accdekomValue = $this->nilai6Model
        ...
        // Mengambil nilai accdekom untuk faktor6id 1 hingga 9
        $accdekomApproved = true;
        for ($faktor6Id = 1; $faktor6Id <= 9; $faktor6Id++) {
            $nilaiAccdekom = $this->nilai6Model
                ->where('faktor6id', $faktor6Id)
        ...
        $faktorData = $this->faktor6Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor6id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 10); // Membuat array dari 1 hingga 10
        foreach ($requiredFaktorIds as $faktor6Id) {
            $associatedNilai = $this->nilai6Model
                ->where('faktor6id', $faktor6Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor6Id) {
        //     // Ambil nilai untuk faktor6id
        //     $associatedNilai = $this->nilai6Model->where('faktor6id', $faktor6Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai6Model->hitungRataRata($faktor6Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai6Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai6Model->insertOrUpdateRataRata($rataRata, $faktor6Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor6Id' => $faktor6Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads6Model' => $this->commentReads6Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor6Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor6Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor6Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor6Id)) {
            // log_message('debug', 'Invalid AJAX or faktor6Id: ' . $faktor6Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai6Model->getNilaiByFaktorId($faktor6Id);

        // log_message('debug', 'Comments returned for faktor6Id ' . $faktor6Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor6Id = $this->request->getPost('faktor6_id');
                $data = [
                    'faktor6id' => $faktor6Id,
                ...
                $this->nilai6Model->tambahNilai($data, $faktor6Id, $kodebpr);

                // Calculate average value from faktor6id 1–8
                $rataRata = $this->nilai6Model->hitungRataRata($faktor6Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor6id 1 in nfaktor column (insert or update)
                $this->nilai6Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor6') . '?modal_nilai=' . $faktor6Id);
            }
        } else {
            return redirect()->to(base_url('faktor6'));
        }
    }
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor6') . '?modal_komentar=' . $this->request->getPost('faktor6_id'));
        }
    ...
    {
        $faktor6id = $this->request->getPost('faktor6id');
        $userId = service('authentication')->id();
        ...
        // Pastikan update berdasarkan faktor6id, kodebpr, dan periode_id
        if ($this->nilai6Model->ubahBerdasarkanFaktorId($data, $faktor6id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor6id 10
            $this->nilai6Model->where('faktor6id', 10)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    {
        $data = [
            'faktor6' => $this->faktor6Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor6Model
        ];

        echo view('faktor6/excel', $data);
    }
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor6Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor6Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor6_id
        $nilaiFaktor6 = $this->nilai6Model->find($idNilai);
        if (!$nilaiFaktor6) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor6 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai6Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai6Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai6Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor6 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai6Model
                ->where('faktor6id', 10)  // Only update where faktor6id = 10
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor6');
                return redirect()->back();
            }
    ...
    public function accdekom()
    {
        ...
        if (!$faktor6id || !$kodebpr || !$periodeId) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap']);
        }

        // Ambil data faktor6 berdasarkan faktor6id, kodebpr, dan periode_id
        $nilaiFaktor6 = $this->nilai6Model
            ->where('faktor6id', $faktor6id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->first();
        ...
        $this->nilai6Model->where('faktor6id', 10)->update(null, $dataUpdateForFaktor10);
        ...
        if ($this->nilai6Model->update($nilaiFaktor6['id'], $dataUpdate)) {
            return redirect()->to('/faktor6')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor6')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function unapprovedekom()
    {
        ...
        // Update record dalam database
        if ($this->nilai6Model->update($nilaiFaktor6['id'], $dataUpdate)) {
            // Update accdekom untuk faktor6id 10
            $this->nilai6Model->where('faktor6id', 10)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->set(['accdekom' => 0])
            ...
            return redirect()->to('/faktor6')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor6')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor6id 1 to 9
        $allApproved = true;
        for ($faktor6Id = 1; $faktor6Id <= 9; $faktor6Id++) {
            $nilaiFaktor6 = $this->nilai6Model
                ->where('faktor6id', $faktor6Id)
            ...
            if (!$nilaiFaktor6 || $nilaiFaktor6['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor6Id = $this->request->getGet('faktor6_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor6Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads6Model->countUnreadCommentsForUserByFactor($faktor6Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads6Model->markAsRead($comment['id'], $userId);
            }
        }
