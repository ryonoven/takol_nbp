...
class Faktor6 extends Controller
{
    ...
    public function approve($idFaktor6)
    {
        if (!is_numeric($idFaktor6) || $idFaktor6 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
        }

        $faktor = $this->faktor6Model->find($idFaktor6);
        if (!$faktor) {
        ...
        if ($this->faktor6Model->save($dataUpdate)) {
    ...
    public function unapprove($idFaktor6)
    {
        if (!is_numeric($idFaktor6) || $idFaktor6 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
    ...
    public function approveSemua()
    {
        ...
        // Update all records
        if ($this->faktor6Model->builder()->update($dataUpdate)) {
