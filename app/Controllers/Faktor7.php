...
use App\Models\M_faktor7komentar;
use App\Models\M_nilaifaktor7;
use App\Models\M_infobpr;
use App\Models\M_periode;
use App\Models\M_commentreads7;
use Myth\Auth\Config\Services as AuthServices;

class Faktor7 extends Controller
{
    protected $auth;
    ...
    public function __construct()
    {
        ...
        $this->komentarModel = new M_faktor7komentar();
        $this->nilai7Model = new M_nilaifaktor7();
        $this->infobprModel = new M_infobpr();
        $this->commentReads7Model = new M_commentreads7();
        helper('url');
    ...
    public function index()
    {
        ...
        // Loop untuk memeriksa faktor7id dari 1 hingga 9
        for ($faktor7Id = 1; $faktor7Id <= 12; $faktor7Id++) {
            $accdekomValue = $this->nilai7Model
        ...
        // Mengambil nilai accdekom untuk faktor7id 1 hingga 9
        $accdekomApproved = true;
        for ($faktor7Id = 1; $faktor7Id <= 9; $faktor7Id++) {
            $nilaiAccdekom = $this->nilai7Model
                ->where('faktor7id', $faktor7Id)
        ...
        $faktorData = $this->faktor7Model->getAllData();
        $factorsWithDetails = [];

        // Memeriksa apakah nilai sudah disetujui untuk faktor7id 1-8
        $allApproved = true;
        $requiredFaktorIds = range(1, 10); // Membuat array dari 1 hingga 10
        foreach ($requiredFaktorIds as $faktor7Id) {
            $associatedNilai = $this->nilai7Model
                ->where('faktor7id', $faktor7Id)
                ->where('kodebpr', $kodebpr)
                ->first();
        ...
        // foreach ($requiredFaktorIds as $faktor7Id) {
        //     // Ambil nilai untuk faktor7id
        //     $associatedNilai = $this->nilai7Model->where('faktor7id', $faktor7Id)->first();
        ...
        foreach ($faktorData as $faktorItem) {
            ...
            $rataRata = $this->nilai7Model->hitungRataRata($faktor7Id, $kodebpr);

            // Get explanation for the calculated average value (rata-rata)
            $penjelasfaktor = $this->nilai7Model->getPenjelasanNilai($rataRata);

            // Insert or update the calculated average value (rata-rata) into the database
            $this->nilai7Model->insertOrUpdateRataRata($rataRata, $faktor7Id, $kodebpr);
        ...
        $data = [
            ...
            // 'faktor7Id' => $faktor7Id, // Tidak perlu dikirim ke view jika tidak digunakan
            'fullname' => $fullname,
            'allApproved' => $allApproved,
            'kodebpr' => $this->userKodebpr,
            'nilaiData' => $nilaiData,
            'komentarModel' => $this->komentarModel,
            'commentReads7Model' => $this->commentReads7Model,
            'lastVisit' => $lastVisit,
            'periodeId' => $periodeId,
    ...
    public function cekKomentarBaru()
    {
        ...
        $results = $this->komentarModel
            ->select('faktor7id, COUNT(*) as jumlah')
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
    ...
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor7Id)) {
        ...
        $komentarList = $this->komentarModel->getKomentarByFaktorId($faktor7Id, $kodebpr, $periodeId);

        return $this->response->setJSON($komentarList);
    }

    public function getNilaiByFaktorId($faktor7Id)
    {
        if (!$this->request->isAJAX() || !is_numeric($faktor7Id)) {
            // log_message('debug', 'Invalid AJAX or faktor7Id: ' . $faktor7Id); // Tambahkan log untuk debug
            return $this->response->setStatusCode(404)->setBody('Not Found');
        }

        $nilaiList = $this->nilai7Model->getNilaiByFaktorId($faktor7Id);

        // log_message('debug', 'Comments returned for faktor7Id ' . $faktor7Id . ': ' . json_encode($komentarList)); // Tambahkan log untuk debug
    ...
    public function tambahNilai()
    {
        ...
        if (isset($_POST['tambahNilai'])) {
            ...
            if (!$val) {
                ...
                $faktor7Id = $this->request->getPost('faktor7_id');
                $data = [
                    'faktor7id' => $faktor7Id,
                ...
                $this->nilai7Model->tambahNilai($data, $faktor7Id, $kodebpr);

                // Calculate average value from faktor7id 1–8
                $rataRata = $this->nilai7Model->hitungRataRata($faktor7Id, $kodebpr);
                $rataRata = ($rataRata - floor($rataRata) >= 0.5) ? ceil($rataRata) : floor($rataRata);

                // Save to faktor7id 1 in nfaktor column (insert or update)
                $this->nilai7Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr);

                session()->setFlashdata('message', 'Nilai berhasil ditambahkan');
                return redirect()->to(base_url('faktor7') . '?modal_nilai=' . $faktor7Id);
            }
        } else {
            return redirect()->to(base_url('faktor7'));
        }
    }
    ...
    public function tambahKomentar()
    {
        ...
        if (isset($_POST['tambahKomentar'])) {
            ...
            return redirect()->to(base_url('faktor7') . '?modal_komentar=' . $this->request->getPost('faktor7_id'));
        }
    ...
    {
        $faktor7id = $this->request->getPost('faktor7id');
        $userId = service('authentication')->id();
        ...
        // Pastikan update berdasarkan faktor7id, kodebpr, dan periode_id
        if ($this->nilai7Model->ubahBerdasarkanFaktorId($data, $faktor7id, $kodebpr, $periodeId)) {
            // Update accdekom dan is_approved untuk faktor7id 10
            $this->nilai7Model->where('faktor7id', 10)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
    ...
    {
        $data = [
            'faktor7' => $this->faktor7Model->getAllData() // <<< PERBAIKI DI SINI: $this->model -> $this->faktor7Model
        ];

        echo view('faktor7/excel', $data);
    }
    ...
    public function setNullKolom($id)
    {
        ...
        $success = $this->faktor7Model->setNullKolom($id);
    ...
    // Fungsi approve dan unapprove sudah menggunakan $this->faktor7Model dengan benar
    public function approve($idNilai = null)
    {
        ...
        // Ambil data dari tabel nilaifaktor berdasarkan faktor7_id
        $nilaiFaktor7 = $this->nilai7Model->find($idNilai);
        if (!$nilaiFaktor7) {
            session()->setFlashdata('err', 'Data tidak ditemukan.');
            return redirect()->back();
        }
        // Memeriksa apakah nilai faktor7 sudah diapprove atau tidak
        date_default_timezone_set('Asia/Jakarta');
        $userId = service('authentication')->id();
        ...
        if ($this->nilai7Model->update($idNilai, $dataUpdate)) {
    ...
    public function approveSemua()
    {
        ...
        try {
            ...
            $rataRata = $this->nilai7Model->hitungRataRata(1, $kodebpr, $periodeId);
            $this->nilai7Model->insertOrUpdateRataRata($rataRata, 1, $kodebpr, $periodeId);

            session()->setFlashdata('message', 'Semua faktor7 berhasil disetujui.');
            return redirect()->back();
    ...
    public function unapprovekom()
    {
        ...
        try {
            $updated = $this->nilai7Model
                ->where('faktor7id', 10)  // Only update where faktor7id = 10
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->update(null, $dataUpdate); // Parameter pertama null untuk update semua yang sesuai kriteria

            if (!$updated) {
                session()->setFlashdata('err', 'Gagal mengupdate data approval untuk faktor7');
                return redirect()->back();
            }
    ...
    public function accdekom()
    {
        ...
        if (!$faktor7id || !$kodebpr || !$periodeId) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap']);
        }

        // Ambil data faktor7 berdasarkan faktor7id, kodebpr, dan periode_id
        $nilaiFaktor7 = $this->nilai7Model
            ->where('faktor7id', $faktor7id)
            ->where('kodebpr', $kodebpr)
            ->where('periode_id', $periodeId)
            ->first();
        ...
        $this->nilai7Model->where('faktor7id', 12)->update(null, $dataUpdateForFaktor12);
        ...
        if ($this->nilai7Model->update($nilaiFaktor7['id'], $dataUpdate)) {
            return redirect()->to('/faktor7')->with('message', 'Faktor berhasil disetujui');
        } else {
            return redirect()->to('/faktor7')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function unapprovedekom()
    {
        ...
        // Update record dalam database
        if ($this->nilai7Model->update($nilaiFaktor7['id'], $dataUpdate)) {
            // Update accdekom untuk faktor7id 10
            $this->nilai7Model->where('faktor7id', 10)
                ->where('kodebpr', $kodebpr)
                ->where('periode_id', $periodeId)
                ->set(['accdekom' => 0])
            ...
            return redirect()->to('/faktor7')->with('message', 'Faktor belum disetujui dekom');
        } else {
            return redirect()->to('/faktor7')->with('error', 'Gagal memperbarui data');
        }
    ...
    public function checkAccDekomApproval()
    {
        ...
        // Check if accdekom is 1 for faktor7id 1 to 9
        $allApproved = true;
        for ($faktor7Id = 1; $faktor7Id <= 9; $faktor7Id++) {
            $nilaiFaktor7 = $this->nilai7Model
                ->where('faktor7id', $faktor7Id)
            ...
            if (!$nilaiFaktor7 || $nilaiFaktor7['accdekom'] != 1) {
                $allApproved = false;
                break;
            }
        }
    ...
    public function getUnreadCommentCountForFactor()
    {
        ...
        $faktor7Id = $this->request->getGet('faktor7_id');
        $kodebpr = $this->userKodebpr;
        $userId = user_id();
        $periodeId = session('active_periode');

        if (!$faktor7Id || !$kodebpr || !$userId || !$periodeId) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Missing data.']);
        }

        $count = $this->commentReads7Model->countUnreadCommentsForUserByFactor($faktor7Id, $kodebpr, $userId, $periodeId);
    ...
    public function markUserCommentsAsRead()
    {
        ...
        if (!empty($commentsToMark)) {
            foreach ($commentsToMark as $comment) {
                $this->commentReads7Model->markAsRead($comment['id'], $userId);
            }
        }
