...
class Faktor7 extends Controller
{
    ...
    public function approve($idFaktor7)
    {
        if (!is_numeric($idFaktor7) || $idFaktor7 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
        }

        $faktor = $this->faktor7Model->find($idFaktor7);
        if (!$faktor) {
        ...
        if ($this->faktor7Model->save($dataUpdate)) {
    ...
    public function unapprove($idFaktor7)
    {
        if (!is_numeric($idFaktor7) || $idFaktor7 <= 0) {
            session()->setFlashdata('err', 'ID Faktor tidak valid.');
            return redirect()->back();
    ...
    public function approveSemua()
    {
        ...
        // Update all records
        if ($this->faktor7Model->builder()->update($dataUpdate)) {
