    ...
    <div class="card shadow">
        ...
        <div class="card-body">
                    ...
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <div class="btn-group" role="group" aria-label="Button group">
                            <button type="button" class="btn btn-primarys btn-sm" data-toggle="modal"
                                data-target="#modalTambahdekom"><i class="fa fa-plus"></i> Tambah Data Kehadiran Anggota
            ...
            <table class="table table-bordered table-hover">
                <tbody>
                    <?php if (
                        empty($kehadirandekom) || !array_filter($kehadirandekom, function ($row) {
                        return !empty($row['dekom']) && !is_null($row['dekom']) &&
                            !empty($row['nik']) && !is_null($row['nik']) &&
                            !empty($row['hadirfisik']) && !is_null($row['hadirfisik']) &&
                            !empty($row['hadironline']) && !is_null($row['hadironline']) &&
                            !empty($row['persen']) && !is_null($row['persen']);
                    })

                    ) { ?>
                        <tr>
                            <td colspan="3" class="text-center">
                                <em>Tidak ada data Kehadiran Anggota Dewan Komisaris</em>
                            </td>
                        </tr>
                    <?php } else { ?>
                        <?php
                        $i = 1;
                        foreach ($kehadirandekom as $row):
                            // Cek apakah data penting tidak kosong dan tidak null
                            if (
                                !empty($row['dekom']) && !is_null($row['dekom']) &&
                                !empty($row['nik']) && !is_null($row['nik']) &&
                                !empty($row['hadirfisik']) && !is_null($row['hadirfisik']) &&
                                !empty($row['hadironline']) && !is_null($row['hadironline']) &&
                                !empty($row['persen']) && !is_null($row['persen'])
                                ...
                                <tr>
                                    ...
                                    <th style="width: 25%; color: black;">Nama Anggota Dewan Komisaris :</th>
                                    <td><?= htmlspecialchars($row['dekom']); ?></td>
                                </tr>
                                <tr>
                                    <th>NIK :</th>
                                ...
                                <tr>
                                    <th>Frekuensi Kehadiran Fisik :</th>
                                    <td><?= htmlspecialchars($row['hadirfisik']); ?></td>
                                </tr>
                                <tr>
                                    <th>Frekuensi Kehadiran Online :</th>
                                    <td><?= htmlspecialchars($row['hadironline']); ?></td>
                                </tr>
                                <tr>
                                    ...
                                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                                        <td colspan="3">
                                            <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                                data-target="#modalUbahhadir" id="btn-edit" data-id="<?= $row['id']; ?>"
                                                data-dekom="<?= htmlspecialchars($row['dekom']); ?>"
                                                data-nik="<?= htmlspecialchars($row['nik']); ?>"
                                                data-hadirfisik="<?= htmlspecialchars($row['hadirfisik']); ?>"
                                                data-hadironline="<?= htmlspecialchars($row['hadironline']); ?>"
                        ...
                        <div class="card shadow-sm approval-card">
                            <div class="card-body approval-card-body">
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Kehadirandekom/approveSemuaKom') ?>"
                                    ...
                                    <a href="<?= base_url('Kehadirandekom/unapproveSemuaKom') ?>"
                    ...
                    <div class="col-md-3">
                        <div class="card shadow-sm approvaldir-card">
                            <div
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Kehadirandekom/approveSemuaDirut') ?>"
                                    ...
                                    <a href="<?= base_url('Kehadirandekom/unapproveSemuaDirut') ?>"
            ...
            <div class="d-flex justify-content-center mb-5">
                <div class="d-flex justify-content-center gap-2 mb-5">
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <a href="<?= base_url('Kehadirandekom/exporttxtkehadirandekom'); ?>"
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group me-2" role="group" aria-label="First group">
                                <button type="button" class="btn btn-outline-primary btn-sm"
                                    onclick="window.location.href='<?= base_url('Rapat') ?>'">
                                    << </button>
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                        ...
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="window.location.href='<?= base_url('Fraudinternal') ?>'">>></button>
                            </div>
                        </div>
                    </div>
                    <div class="text-center mt-1">
                        <a href="<?= base_url('Fraudinternal'); ?>" class="btn btn-link btn-sm">Kembali ke halaman
                            periode</a>
                    </div>
                </div>
            </div>
...
<?php if (!empty($kehadirandekom)): ?>
    <div class="modal fade" id="modalUbahhadir">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?= base_url('Kehadirandekom/ubahhadir'); ?>" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" name="dekom" id="dekom"
                                value="<?= isset($row['dekom']) ? esc($row['dekom']) : ''; ?>" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" name="hadirfisik" id="hadirfisik"
                                value="<?= isset($row['hadirfisik']) ? esc($row['hadirfisik']) : ''; ?>" maxlength="16"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="hadironline" class="form-label">Frekuensi Kehadiran Online:</label>
                            <input class="form-control" type="text" name="hadironline" id="hadironline"
                                value="<?= isset($row['hadironline']) ? esc($row['hadironline']) : ''; ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="persen" class="form-label">Input Persentase Kepemilikan (%) Tahun
                                Sebelumnya : </label>
                            <input class="form-control" type="text" name="persen" id="persen"
                                placeholder="<?= isset($row['persen']) ? esc($row['persen']) : ''; ?>" required></input>
                            <small id="persen" class="form-text text-muted">Gunakan titik jika ada pemisah bilangan.
                                (Contoh: 3.72)"</small>
                        </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" name="ubahhadir" class="btn btn-primary">Ubah Data</button>
...
<div class="modal fade" id="modalTambahketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('kehadirandekom/tambahketerangan'); ?>" method="post">
...
<div class="modal fade" id="modaleditketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('kehadirandekom/editketerangan'); ?>" method="post">
...
<div class="modal fade" id="modalTambahdekom">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah data Kehadiran Anggota Dewan Komisaris</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('Kehadirandekom/tambahdekom'); ?>" method="post">
                    <div class="form-group">
                        <label for="dekom">Nama Anggota Dewan Komisaris:</label>
                    ...
                    <div class="form-group">
                        <label for="hadironline">Frekuensi Kehadiran Online: </label>
                        <input type="text" name="hadironline" id="hadironline" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="persen">Persentase Kehadiran: </label>
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    ...
                    window.location.href = "/kehadirandekom/setNullKolomTindak/" + id;
                }
            });
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    window.location.href = "/Kehadirandekom/setNullKolomPenjelaslanjut/" + id;
                }
            });
        });
    });
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function fetchAndDisplayComments(Id, kodebpr, periodeId) {
            ...
            $.ajax({
                url: '<?= base_url('kehadirandekom/getKomentarByFaktorId'); ?>/' + Id,
                method: 'GET',
                data: {
                    kodebpr: kodebpr,
        ...
        commentButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                $.ajax({
                    url: '<?= base_url('kehadirandekom/markUserCommentsAsRead'); ?>',
                    method: 'POST',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function pollUnreadCounts() {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('kehadirandekom/getUnreadCommentCountForFactor'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function refreshBadgeForAllUsers(exceptUserId) {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('kehadirandekom/getUnreadCommentCountForAllUsers'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
