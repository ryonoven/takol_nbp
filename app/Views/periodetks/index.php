            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="headingOne">
                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                        ...
                        Buat Periode Baru Laporan Tingkat Kesehatan Bank (TKS)
                    </button>
                </h2>
                <div id="collapseOne" class="accordion-collapse collapse" aria-labelledby="headingOne"
                    data-bs-parent="#accordionExample">
                    <div class="accordion-body">
                        <form action="<?= base_url('periodetks/handlePeriode') ?>" method="post" novalidate>
        ...
        <div class="col-md-12">
            <div class="card shadow-lg">
                <div class="card-header bg-info text-white">
                    <h4 class="mb-0">Pilih Periode Pelaporan Tingkat Kesehatan Bank (TKS) Yang Sudah Ada</h4>
                </div>
                <div class="card-body">
                    ...
                    <?php else: ?>
                        <div class="list-group">
                            <?php foreach ($periodes as $periodetks): ?>
                                <form action="<?= base_url('periodetks/handlePeriode') ?>" method="post" class="mb-2">
                                    <input type="hidden" name="action" value="select">
                                    <input type="hidden" name="periode_id" value="<?= $periodetks['id'] ?>">

                                    <button type="submit"
                                        class="list-group-item list-group-item-action <?= ($current_periode == $periodetks['id']) ? 'active' : '' ?>">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <strong style="font-size: 20px;">Periode Pelaporan Semester
                                                    <?= esc($periodetks['semester']) ?>
                                                    Tahun <?= esc($periodetks['tahun']) ?>
                                                </strong><br>
                                                <small style="font-size: 13px;">Dibuat:
                                                    <?= date('d M Y', strtotime($periodetks['created_at'])) ?>
                                                </small><br>
                                                <strong style="font-size: 15px;">Modal Inti:
                                                    <?= "Rp " . number_format(esc($periodetks['modalinti']), 0, ',', '.') ?></strong>
                                                <br>
                                                <strong style="font-size: 15px;">Total Aset:
                                                    <?= "Rp " . number_format(esc($periodetks['totalaset']), 0, ',', '.') ?></strong>
                                                <br>                                                
                                            </div>
                                            <?php if ($current_periode == $periodetks['id']): ?>
