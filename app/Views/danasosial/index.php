...
<div class="container-fluid">
    ...
    <div class="card shadow">
        <div class="card-headertitle">
            ...
            <h1 class="h3 mb-4 text-gray-800 text-center"><?= $judul; ?><br>(E01100)</h1>
        </div>
        ...
        <div class="card-body">
                    ...
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <div class="btn-group" role="group" aria-label="Button group">
                            <button type="button" class="btn btn-primarys btn-sm" data-toggle="modal"
                                data-target="#modalTambahsahamdir"><i class="fa fa-plus"></i> Pemberian Dana untuk Kegiatan
                                Sosial dan Kegiatan Politik</button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <table class="table table-bordered table-hover">
                <tbody>
                    <?php if (
                        empty($danasosial) || !array_filter($danasosial, function ($row) {
                        return !empty($row['tanggalpelaksanaan']) && !is_null($row['tanggalpelaksanaan']) &&
                            !empty($row['jeniskegiatan']) && !is_null($row['jeniskegiatan']) &&
                            !empty($row['penerimadana']) && !is_null($row['penerimadana']) &&
                            !empty($row['penjelasankegiatan']) && !is_null($row['penjelasankegiatan']) &&
                            !empty($row['jumlah']) && !is_null($row['jumlah']);
                    })
                    ) { ?>
                        <tr>
                    ...
                    <?php } else { ?>
                        <?php
                        $i = 1;
                        foreach ($danasosial as $row):
                            // Cek apakah data penting tidak kosong dan tidak null
                            if (
                                !empty($row['tanggalpelaksanaan']) && !is_null($row['tanggalpelaksanaan']) &&
                                !empty($row['jeniskegiatan']) && !is_null($row['jeniskegiatan']) &&
                                !empty($row['penerimadana']) && !is_null($row['penerimadana']) &&
                                !empty($row['penjelasankegiatan']) && !is_null($row['penjelasankegiatan']) &&
                                !empty($row['jumlah']) && !is_null($row['jumlah'])
                                ...
                                <tr>
                                    ...
                                    <th style="width: 25%; color: black;">Tanggal Pelaksanaan :</th>
                                    <td><?= date('d F Y', strtotime($row['tanggalpelaksanaan'])); ?></td>
                                </tr>
                                <tr>
                                    <th>Jenis Kegiatan :</th>
                                    <td>
                                        <?php
                                        // Cek nilai dari database dan tampilkan label yang sesuai
                                        if (isset($row['jeniskegiatan'])) {
                                            if ($row['jeniskegiatan'] == '01') {
                                                echo 'Kegiatan Sosial';
                                            } elseif ($row['jeniskegiatan'] == '02') {
                                                echo 'Kegiatan Politik';
                                            } else {
                                                echo 'Jenis kegiatan tidak diketahui';
                                            }
                                        }
                                        ?>
                                    </td>
                                ...
                                <tr>
                                    <th>Penerima Dana :</th>
                                    <td><?= htmlspecialchars($row['penerimadana']); ?></td>
                                </tr>
                                <tr>
                                    <th>Penjelasan Kegiatan :</th>
                                    <td><?= htmlspecialchars($row['penjelasankegiatan']); ?></td>
                                </tr>
                                <tr>
                                    <th>Total dana :</th>
                                    ...
                                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                                        <td colspan="3">
                                            <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                                ...
                                                data-tanggalpelaksanaan="<?= htmlspecialchars($row['tanggalpelaksanaan']); ?>"
                                                data-jeniskegiatan="<?= htmlspecialchars($row['jeniskegiatan']); ?>"
                                                data-penerimadana="<?= htmlspecialchars($row['penerimadana']); ?>"
                                                data-penjelasankegiatan="<?= htmlspecialchars($row['penjelasankegiatan']); ?>"
                        ...
                        <div class="card shadow-sm approval-card">
                            <div class="card-body approval-card-body">
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Danasosial/approveSemuaKom') ?>"
                                    ...
                                    <a href="<?= base_url('Danasosial/unapproveSemuaKom') ?>"
                    ...
                    <div class="col-md-3">
                        <div class="card shadow-sm approvaldir-card">
                            <div
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Danasosial/approveSemuaDirut') ?>"
                                    ...
                                    <a href="<?= base_url('Danasosial/unapproveSemuaDirut') ?>"
            ...
            <div class="d-flex justify-content-center mb-5">
                <div class="d-flex justify-content-center gap-2 mb-5">
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <a href="<?= base_url('danasosial/exporttxtdanasosial'); ?>" class="btn btn-secondary btn-sm">
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group me-2" role="group" aria-label="First group">
                                <button type="button" class="btn btn-outline-primary btn-sm"
                                    onclick="window.location.href='<?= base_url('Transaksikepentingan') ?>'">
                                    << </button>
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                        ...
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="window.location.href='<?= base_url('ShowTransparansi') ?>'">>></button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ubah data Pemberian Dana untuk Kegiatan Sosial dan Kegiatan Politik </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="<?= base_url('danasosial/ubahdata'); ?>" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="date" name="tanggalpelaksanaan" id="tanggalpelaksanaan"
                                value="<?= isset($row['tanggalpelaksanaan']) ? esc($row['tanggalpelaksanaan']) : ''; ?>"
                                required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="jeniskegiatan" id="jeniskegiatan" required>
                                <option value="01" <?= isset($row['jeniskegiatan']) && $row['jeniskegiatan'] == '01' ? 'selected' : ''; ?>>Kegiatan Sosial</option>
                                <option value="02" <?= isset($row['jeniskegiatan']) && $row['jeniskegiatan'] == '02' ? 'selected' : ''; ?>>Kegiatan Politik</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="penerimadana" class="form-label">Penerima Dana:</label>
                            <input class="form-control" type="text" name="penerimadana" id="penerimadana"
                                value="<?= isset($row['penerimadana']) ? esc($row['penerimadana']) : ''; ?>" maxlength="16"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="penjelasankegiatan" class="form-label">Penjelasan Kegiatan:</label>
                            <input class="form-control" type="text" name="penjelasankegiatan" id="penjelasankegiatan"
                                value="<?= isset($row['penjelasankegiatan']) ? esc($row['penjelasankegiatan']) : ''; ?>"
                                required>
                        </div>
                        <div class="form-group"> <label for="jumlah" class="form-label">Total dana: </label>
                            <div class="input-group"> <span class="input-group-text">Rp</span> <input class="form-control"
                                    type="text" name="jumlah" id="jumlah"
                                    placeholder="<?= isset($row['jumlah']) ? esc($row['jumlah']) : ''; ?>" required></input>
                            </div>
                        </div>
...
<div class="modal fade" id="modalTambahketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('danasosial/tambahketerangan'); ?>" method="post">
...
<div class="modal fade" id="modaleditketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('danasosial/editketerangan'); ?>" method="post">
...
<div class="modal fade" id="modalTambahsahamdir">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah data Pemberian Dana untuk Kegiatan Sosial dan Kegiatan Politik</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('danasosial/tambahdanasosial'); ?>" method="post">
                    <div class="form-group">
                        <label for="tanggalpelaksanaan">Tanggal pelaksanaan kegiatan:</label>
                        <input type="date" name="tanggalpelaksanaan" id="tanggalpelaksanaan" class="form-control"
                            required>
                    </div>
                    <div class="mb-3">
                        <label for="jeniskegiatan" class="form-label">Jenis kegiatan:</label>
                        <select class="form-control" name="jeniskegiatan" id="jeniskegiatan" required>
                            ...
                            <option value="01">Kegiatan Sosial</option>
                            <option value="02">Kegiatan Politik</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="penerimadana">Penerima dana: </label>
                        <input type="text" name="penerimadana" id="penerimadana" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="penjelasankegiatan">Penjelasan kegiatan: </label>
                        <input type="text" name="penjelasankegiatan" id="penjelasankegiatan" class="form-control"
                            required>
                    </div>
                    <div class="form-group">
                        <label for="jumlah">Total dana: </label>
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    ...
                    window.location.href = "/danasosial/setNullKolomTindak/" + id;
                }
            });
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    window.location.href = "/danasosial/setNullKolomPenjelaslanjut/" + id;
                }
            });
        });
    });
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function fetchAndDisplayComments(Id, kodebpr, periodeId) {
            ...
            $.ajax({
                url: '<?= base_url('danasosial/getKomentarByFaktorId'); ?>/' + Id,
                method: 'GET',
                data: {
                    kodebpr: kodebpr,
        ...
        commentButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                $.ajax({
                    url: '<?= base_url('danasosial/markUserCommentsAsRead'); ?>',
                    method: 'POST',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function pollUnreadCounts() {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('danasosial/getUnreadCommentCountForFactor'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function refreshBadgeForAllUsers(exceptUserId) {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('danasosial/getUnreadCommentCountForAllUsers'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
