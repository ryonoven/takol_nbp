...
<div class="container-fluid">
    ...
    <div class="card shadow">
        <div class="card-headertitle">
            ...
            <h1 class="h3 mb-4 text-gray-800 text-center"><?= $judul; ?><br>(E1000)</h1>
        </div>
        ...
        <div class="card-body">
                    ...
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <div class="btn-group" role="group" aria-label="Button group">
                            <button type="button" class="btn btn-primarys btn-sm" data-toggle="modal"
                                data-target="#modalTambahtransaksi"><i class="fa fa-plus"></i> Tambah Data Kehadiran
                                Anggota
                                Dewan Komisaris</button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <table class="table table-bordered table-hover">
                <tbody>
                    <?php if (
                        empty($transaksikepentingan) || !array_filter($transaksikepentingan, function ($row) {
                        return !empty($row['namapihakbenturan']) && !is_null($row['namapihakbenturan']) &&
                            !empty($row['jbtbenturan']) && !is_null($row['jbtbenturan']) &&
                            !empty($row['nikbenturan']) && !is_null($row['nikbenturan']) &&
                            !empty($row['pengambilkeputusan']) && !is_null($row['pengambilkeputusan']) &&
                            !empty($row['jbtpengambilkeputusan']) && !is_null($row['jbtpengambilkeputusan']) &&
                            !empty($row['nikpengambilkeputusan']) && !is_null($row['nikpengambilkeputusan']) &&
                            !empty($row['jenistransaksi']) && !is_null($row['jenistransaksi']) &&
                            !empty($row['nilaitransaksi']) && !is_null($row['nilaitransaksi']) &&
                            !empty($row['tindakbenturan']) && !is_null($row['tindakbenturan']);
                    })

                    ) { ?>
                        <tr>
                            <td colspan="3" class="text-center">
                                <em>Tidak ada data Transaksi yang Mengandung Benturan Kepentingan</em>
                            </td>
                        </tr>
                    <?php } else { ?>
                        <?php
                        $i = 1;
                        foreach ($transaksikepentingan as $row):
                            // Cek apakah data penting tidak kosong dan tidak null
                            if (
                                !empty($row['namapihakbenturan']) && !is_null($row['namapihakbenturan']) &&
                                !empty($row['jbtbenturan']) && !is_null($row['jbtbenturan']) &&
                                !empty($row['nikbenturan']) && !is_null($row['nikbenturan']) &&
                                !empty($row['pengambilkeputusan']) && !is_null($row['pengambilkeputusan']) &&
                                !empty($row['jbtpengambilkeputusan']) && !is_null($row['jbtpengambilkeputusan']) &&
                                ...
                                ?>
                                <tr>
                                    <th class="table-info" style="width: 3%;" rowspan="10"><?= $i++; ?></th>
                                    <th style="width: 25%; color: black;">Nama Pihak yang Memiliki Benturan Kepentingan :</th>
                                    <td><?= htmlspecialchars($row['namapihakbenturan']); ?></td>
                                </tr>
                                <tr>
                                    <th>Jabatan Pihak yang Memiliki Benturan Kepentingan :</th>
                                    <td><?= htmlspecialchars($row['jbtbenturan']); ?></td>
                                </tr>
                                <tr>
                                    <th>NIK Pihak yang Memiliki Benturan Kepentingan :</th>
                                    <td><?= htmlspecialchars($row['nikbenturan']); ?></td>
                                </tr>
                                <tr>
                                    <th>Nama Pengambil Keputusan :</th>
                                    <td><?= htmlspecialchars($row['pengambilkeputusan']); ?></td>
                                </tr>
                                <tr>
                                    <th>Jabatan Pengambil Keputusan :</th>
                                    <td><?= htmlspecialchars($row['jbtpengambilkeputusan']); ?></td>
                                </tr>
                                <tr>
                                    <th>NIK Pengambil Keputusan :</th>
                                    <td><?= htmlspecialchars($row['nikpengambilkeputusan']); ?></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        Rp <?= number_format($row['nilaitransaksi'], 0, ',', '.'); ?>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Keterangan :</th>
                                    <td><?= htmlspecialchars($row['tindakbenturan']); ?></td>
                                </tr>
                                <tr>
                                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                                        <td colspan="3">
                                            <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                                data-target="#modalUbahdata" id="btn-edit" data-id="<?= $row['id']; ?>"
                                                data-namapihakbenturan="<?= htmlspecialchars($row['namapihakbenturan']); ?>"
                                                data-jbtbenturan="<?= htmlspecialchars($row['jbtbenturan']); ?>"
                                                data-nikbenturan="<?= htmlspecialchars($row['nikbenturan']); ?>"
                                                data-pengambilkeputusan="<?= htmlspecialchars($row['pengambilkeputusan']); ?>"
                                                data-jbtpengambilkeputusan="<?= htmlspecialchars($row['jbtpengambilkeputusan'] ?? ''); ?>"
                                                data-nikpengambilkeputusan="<?= htmlspecialchars($row['nikpengambilkeputusan']); ?>"
                                                data-jenistransaksi="<?= htmlspecialchars($row['jenistransaksi']); ?>"
                                                data-nilaitransaksi="<?= htmlspecialchars($row['nilaitransaksi']); ?>"
                                                data-tindakbenturan="<?= htmlspecialchars($row['tindakbenturan']); ?>">
                        ...
                        <div class="card shadow-sm approval-card">
                            <div class="card-body approval-card-body">
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Transaksikepentingan/approveSemuaKom') ?>"
                                    ...
                                    <a href="<?= base_url('Transaksikepentingan/unapproveSemuaKom') ?>"
                    ...
                    <div class="col-md-3">
                        <div class="card shadow-sm approvaldir-card">
                            <div
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Transaksikepentingan/approveSemuaDirut') ?>"
                                    ...
                                    <a href="<?= base_url('Transaksikepentingan/unapproveSemuaDirut') ?>"
            ...
            <div class="d-flex justify-content-center mb-5">
                <div class="d-flex justify-content-center gap-2 mb-5">
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <a href="<?= base_url('Transaksikepentingan/exporttxttransaksikepentingan'); ?>"
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group me-2" role="group" aria-label="First group">
                                <button type="button" class="btn btn-outline-primary btn-sm"
                                    onclick="window.location.href='<?= base_url('Masalahhukum') ?>'">
                                    << </button>
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                        ...
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="window.location.href='<?= base_url('Danasosial') ?>'">>></button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ubah data Transaksi yang Mengandung Benturan Kepentingan </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="<?= base_url('Transaksikepentingan/ubahdata'); ?>" method="post">
                        <input type="hidden" name="id" id="id-transaksikepentingan"
                            value="<?= isset($row['id']) ? esc($row['id']) : ''; ?>">

                        <div class="mb-3">
                            <label for="namapihakbenturan" class="form-label">Nama Pihak yang Memiliki Benturan
                                Kepentingan:</label>
                            <input class="form-control" type="text" name="namapihakbenturan" id="namapihakbenturan"
                                value="<?= isset($row['namapihakbenturan']) ? esc($row['namapihakbenturan']) : ''; ?>"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="jbtbenturan" class="form-label">Jabatan Pihak yang Memiliki Benturan
                                Kepentingan:</label>
                            <input class="form-control" type="text" name="jbtbenturan" id="jbtbenturan"
                                value="<?= isset($row['jbtbenturan']) ? esc($row['jbtbenturan']) : ''; ?>" maxlength="16"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="nikbenturan" class="form-label">NIK Pihak yang Memiliki Benturan
                                Kepentingan:</label>
                            <input class="form-control" type="text" name="nikbenturan" id="nikbenturan"
                                value="<?= isset($row['nikbenturan']) ? esc($row['nikbenturan']) : ''; ?>" maxlength="16"
                                required>
                            <small id="nikCounterUbah" class="form-text text-muted">0/16 digit</small>
                        </div>
                        <div class="mb-3">
                            <label for="pengambilkeputusan" class="form-label">Nama Pengambil Keputusan:</label>
                            <input class="form-control" type="text" name="pengambilkeputusan" id="pengambilkeputusan"
                                value="<?= isset($row['pengambilkeputusan']) ? esc($row['pengambilkeputusan']) : ''; ?>"
                                maxlength="16" required>
                        </div>
                        <div class="mb-3">
                            <label for="jbtpengambilkeputusan" class="form-label">Jabatan Pengambil Keputusan:</label>
                            <input class="form-control" type="text" name="jbtpengambilkeputusan" id="jbtpengambilkeputusan"
                                value="<?= isset($row['jbtpengambilkeputusan']) ? esc($row['jbtpengambilkeputusan']) : ''; ?>"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="nikpengambilkeputusan" class="form-label">NIK Pihak yang Memiliki Benturan
                                Kepentingan:</label>
                            <input class="form-control" type="text" name="nikpengambilkeputusan" id="nikpengambilkeputusan"
                                value="<?= isset($row['nikpengambilkeputusan']) ? esc($row['nikpengambilkeputusan']) : ''; ?>"
                                maxlength="16" required>
                            <small id="nikCounterUbah" class="form-text text-muted">0/16 digit</small>
                        </div>
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" name="jenistransaksi" id="jenistransaksi"
                                value="<?= isset($row['jenistransaksi']) ? esc($row['jenistransaksi']) : ''; ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="nilaitransaksi" class="form-label">Nilai Transaksi:</label>
                            <div class="input-group">
                                <span class="input-group-text">Rp</span>
                                <input class="form-control" type="text" name="nilaitransaksi" id="nilaitransaksi"
                                    value="<?= isset($row['nilaitransaksi']) ? esc($row['nilaitransaksi']) : ''; ?>"
                                    required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="tindakbenturan" class="form-label">Keterangan:</label>
                            <input class="form-control" type="text" name="tindakbenturan" id="tindakbenturan"
                                value="<?= isset($row['tindakbenturan']) ? esc($row['tindakbenturan']) : ''; ?>" required>
                        </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" name="ubahhadata" class="btn btn-primary">Ubah Data</button>
...
<div class="modal fade" id="modalTambahketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('transaksikepentingan/tambahketerangan'); ?>" method="post">
...
<div class="modal fade" id="modaleditketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('transaksikepentingan/editketerangan'); ?>" method="post">
...
<div class="modal fade" id="modalTambahtransaksi">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah data Transaksi yang Mengandung Benturan Kepentingan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('Transaksikepentingan/tambahtransaksi'); ?>" method="post">
                    <div class="form-group">
                        <label for="namapihakbenturan">Nama Pihak yang Memiliki Benturan Kepentingan:</label>
                        <input type="text" name="namapihakbenturan" id="namapihakbenturan" class="form-control"
                            required>
                    </div>
                    <div class="form-group">
                        <label for="jbtbenturan">Jabatan Pihak yang Memiliki Benturan Kepentingan: </label>
                        <input type="text" name="jbtbenturan" id="jbtbenturan" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label for="nikbenturan" class="form-label">NIK Pihak yang Memiliki Benturan
                            Kepentingan:</label>
                        <input class="form-control" type="text" name="nikbenturan" id="nikbenturanTambah" maxlength="16"
                            required>
                        <small id="nikCounterTambah" class="form-text text-muted">0/16 digit</small>
                    </div>
                    <div class="form-group">
                        <label for="pengambilkeputusan">Nama Pengambil Keputusan: </label>
                        <input type="text" name="pengambilkeputusan" id="pengambilkeputusan" class="form-control"
                            required>
                    </div>
                    <div class="form-group">
                        <label for="jbtpengambilkeputusan">Jabatan Pengambil Keputusan: </label>
                        <input type="text" name="jbtpengambilkeputusan" id="jbtpengambilkeputusan" class="form-control"
                            required>
                    </div>
                    <div class="mb-3">
                        <label for="nikpengambilkeputusan" class="form-label">NIK Pengambil Keputusan:</label>
                        <input class="form-control" type="text" name="nikpengambilkeputusan"
                            id="nikpengambilkeputusanTambah" maxlength="16" required>
                        <small id="nikCounterTambah" class="form-text text-muted">0/16 digit</small>
                    </div>
                    <div class="form-group">
                        <label for="jenistransaksi">Jenis Transaksi: </label>
                        <input type="text" name="jenistransaksi" id="jenistransaksi" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="nilaitransaksi">Nilai Transaksi: </label>
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="text" name="nilaitransaksi" id="nilaitransaksi"
                                class="form-control format-currency" style="height: 45px"
                                value="<?= esc($transaksikepentingan[0]['nilaitransaksi'] ?? '') ?>" requiredy>
                            <input type="hidden" name="nilaitransaksi" id="nilaitransaksi_raw"
                                value="<?= esc($transaksikepentingan[0]['nilaitransaksi'] ?? '') ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="tindakbenturan">Keterangan: </label>
                        <input type="text" name="tindakbenturan" id="tindakbenturanss" class="form-control" required>
                    </div>
            </div>
            <div class="modal-footer">
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    ...
                    window.location.href = "/transaksikepentingan/setNullKolomTindak/" + id;
                }
            });
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    window.location.href = "/Transaksikepentingan/setNullKolomPenjelaslanjut/" + id;
                }
            });
        });
    });
...
<script>
    ...
    handleNikCounter('nikTambahtransaksi', 'nikCounterTambahtransaksi');
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function fetchAndDisplayComments(Id, kodebpr, periodeId) {
            ...
            $.ajax({
                url: '<?= base_url('transaksikepentingan/getKomentarByFaktorId'); ?>/' + Id,
                method: 'GET',
                data: {
                    kodebpr: kodebpr,
        ...
        commentButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                $.ajax({
                    url: '<?= base_url('transaksikepentingan/markUserCommentsAsRead'); ?>',
                    method: 'POST',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function pollUnreadCounts() {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('transaksikepentingan/getUnreadCommentCountForFactor'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function refreshBadgeForAllUsers(exceptUserId) {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('transaksikepentingan/getUnreadCommentCountForAllUsers'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
