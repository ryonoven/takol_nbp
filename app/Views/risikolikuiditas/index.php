    ...
    <div class="card card-body">
        <div class="table-vertical">
            <div class="button-bar">
                <a href="Showprofilresiko" class="kpmr-btn icon-link icon-link-hover" id="btn-laporan">🡸 Laporan Profil
                    Risiko</a>
                <a href="Likuiditaskpmr" class="kpmr-btn" id="btn-risiko">Risiko Likuiditas KPMR 🡺</a>
            </div>
                ...
                <div class="text-center flex-grow-1">
                    <div class="text-center">
                        <h4 class="mb-0">
                            ...
                            <strong>Tingkat Likuiditas Inheren</strong>
                        </h4>
            ...
            <div id="editNilai14" class="collapse">
                <div class="p-3 border-top">
                    <?php if (!empty($nilai14)): ?>
                        ...
                        <form action="<?= base_url('Likuiditasinheren/simpanNilai14') ?>" method="post">
                            <?= csrf_field() ?>
            ...
            <div class="card-body">
                <div class="mb-2">
                    <strong>A. Kertas Kerja Inheren Likuiditas</strong>
                </div>
                <?php if (($userInGroupPE ?? false) || ($userInGroupAdmin ?? false)): ?>
                    <button class="btn btn-primary mr-2 mb-3" type="button" data-bs-toggle="offcanvas"
                        data-bs-target="#offcanvasWithBothOptions" aria-controls="offcanvasWithBothOptions">
                        Kalkulator Risiko Likuiditas Inheren
                    </button>
                <?php endif; ?>
                ...
                <?php if (($userInGroupPE ?? false) || ($userInGroupAdmin ?? false)): ?>
                    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasWithBothOptions"
                        ...
                        <div class="offcanvas-body">
                            <form id="riskForm">
                                <div class="mb-3">
                                    <label for="totalaset" class="form-label">Total Aset</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="totalaset" name="totalaset"
                                            placeholder="<?= !empty($kalkulatorData['totalaset']) ? number_format($kalkulatorData['totalaset'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['totalaset']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="kas" name="kas"
                                            placeholder="<?= !empty($kalkulatorData['kas']) ? number_format($kalkulatorData['kas'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['kas']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="girobanklain" class="form-label">Giro pada bank lain</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="girobanklain"
                                            name="girobanklain"
                                            placeholder="<?= !empty($kalkulatorData['girobanklain']) ? number_format($kalkulatorData['girobanklain'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['girobanklain']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-4">
                                    <label for="tabunganbanklain" class="form-label">Tabungan pada Bank Lain</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="tabunganbanklain"
                                            name="tabunganbanklain"
                                            placeholder="<?= !empty($kalkulatorData['tabunganbanklain']) ? number_format($kalkulatorData['tabunganbanklain'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['tabunganbanklain']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="kewajibansegera" class="form-label">Kewajiban segera</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="kewajibansegera"
                                            name="kewajibansegera"
                                            placeholder="<?= !empty($kalkulatorData['kewajibansegera']) ? number_format($kalkulatorData['kewajibansegera'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['kewajibansegera']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="tabungandpk" class="form-label">Tabungan DPK</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="tabungandpk"
                                            name="tabungandpk"
                                            placeholder="<?= !empty($kalkulatorData['tabungandpk']) ? number_format($kalkulatorData['tabungandpk'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['tabungandpk']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="depositodpk" class="form-label">Deposito DPK</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="depositodpk"
                                            name="depositodpk"
                                            placeholder="<?= !empty($kalkulatorData['depositodpk']) ? number_format($kalkulatorData['depositodpk'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['depositodpk']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="tabunganabp" class="form-label">Tabungan ABP</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="tabunganabp"
                                            name="tabunganabp"
                                            placeholder="<?= !empty($kalkulatorData['tabunganabp']) ? number_format($kalkulatorData['tabunganabp'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['tabunganabp']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="depositoabp" class="form-label">Deposito ABP</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="depositoabp"
                                            name="depositoabp"
                                            placeholder="<?= !empty($kalkulatorData['depositoabp']) ? number_format($kalkulatorData['depositoabp'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['depositoabp']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="pinjamanditerima" class="form-label">Pinjaman yang diterima</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="pinjamanditerima"
                                            name="pinjamanditerima"
                                            placeholder="<?= !empty($kalkulatorData['pinjamanditerima']) ? number_format($kalkulatorData['pinjamanditerima'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['pinjamanditerima']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="kreditkyd" class="form-label">Kredit yang diberikan</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="kreditkyd"
                                            name="kreditkyd"
                                            placeholder="<?= !empty($kalkulatorData['kreditkyd']) ? number_format($kalkulatorData['kreditkyd'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['kreditkyd']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="penabung25deposan" class="form-label">25 deposan dan penabung
                                        terbesar</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="penabung25deposan"
                                            name="penabung25deposan"
                                            placeholder="<?= !empty($kalkulatorData['penabung25deposan']) ? number_format($kalkulatorData['penabung25deposan'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['penabung25deposan']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="totalpendanaan" class="form-label">Total Pendanaan</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="totalpendanaan"
                                            name="totalpendanaan"
                                            placeholder="<?= !empty($kalkulatorData['totalpendanaan']) ? number_format($kalkulatorData['totalpendanaan'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['totalpendanaan']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="dpkdiataslps" class="form-label">DPK di atas LPS</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="dpkdiataslps"
                                            name="dpkdiataslps"
                                            placeholder="<?= !empty($kalkulatorData['dpkdiataslps']) ? number_format($kalkulatorData['dpkdiataslps'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['dpkdiataslps']) ?>" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="pinjamananmungkinditarik" class="form-label">Pinjaman yang Mungkin Ditarik
                                        Sewaktu-Waktu</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" step="0.01" class="form-control" id="pinjamananmungkinditarik"
                                            name="pinjamananmungkinditarik"
                                            placeholder="<?= !empty($kalkulatorData['pinjamananmungkinditarik']) ? number_format($kalkulatorData['kreditkyd'], 0, ',', '.') : 'Masukkan nilai' ?>"
                                            value="<?= esc($kalkulatorData['pinjamananmungkinditarik']) ?>" required>
                                    </div>
                                </div>

                                <!-- Hidden fields for results -->
                                <input type="hidden" id="asetlikuid_hidden">
                                <input type="hidden" id="kewajibanlancar_hidden">
                                <input type="hidden" id="totaldpk_hidden">
                                <input type="hidden" id="transaksibpr_hidden">
                                <input type="hidden" id="pendanaannoninti_hidden">
                                <input type="hidden" id="rasioasetlikuidtotalaset_submit">
                                <input type="hidden" id="rasioasetlikuidkewajiban_submit">
                                <input type="hidden" id="rasiokreditterhadapdpk_submit">
                                <input type="hidden" id="rasiokreditdiberikan_submit">
                                <input type="hidden" id="rasio25deposan_submit">
                                <input type="hidden" id="rasiononinti_submit">

                                <button type="button" class="btn btn-primary w-100" onclick="calculateRiskKalkulator()">
                                    <i class="fas fa-calculator"></i> Hitung
                                </button>
                            </form>

                            <div id="riskResult" class="risk-result mt-3"></div>

                            <div id="detailResult" style="display: none;" class="mt-4">
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td id="kewajibanlancar">-</td>
                                    </tr>
                                    <tr>
                                        <td>Total DPK</td>
                                        <td id="totaldpk_display">-</td>
                                    </tr>
                                    <tr>
                                        <td>Transaksi antar BPR</td>
                                        <td id="transaksibpr">-</td>
                                    </tr>
                                    <tr>
                                        <td>Pendanaan Non Inti</td>
                                        <td id="pendanaannoninti">-</td>
                                    </tr>
                                    <tr>
                                        <td>Rasio aset likuid terhadap total aset</td>
                                        <td id="rasioasetlikuidtotalaset">-</td>
                                    </tr>
                                    <tr>
                                        <td>Rasio aset likuid terhadap kewajiban lancar</td>
                                        <td id="rasioasetlikuidkewajiban">-</td>
                                    </tr>
                                    <tr>
                                        <td>Rasio kredit yang diberikan terhadap total DPK (LDR)</td>
                                        <td id="rasiokreditterhadapdpk">-</td>
                                    </tr>
                                    <tr>
                                        <td>Rasio 25 deposan dan penabung terbesar terhadap total DPK</td>
                                        <td id="rasio25deposan">-</td>
                                    </tr>
                                    <tr>
                                        <td>Rasio pendanaan non inti terhadap total pendanaan</td>
                                        <td id="rasiononinti">-</td>
                                    </tr>
                                </table>

                                <button type="button" id="btnInsertRasio" class="btn btn-success w-100 mt-2"
                                    style="display: none;" onclick="insertRasioToKertasKerja()">
                                    <i class="fas fa-save"></i> Masukkan ke Kertas Kerja
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card mt-3">
                <div class="card-header bg-info text-white">
                    <h6 class="mb-0">
                        <i class="fas fa-clipboard-list"></i> Kesimpulan Penilaian Risiko Likuiditas Inheren
                    </h6>
                </div>
                <div class="card-body">
                    <?php if (!empty($kesimpulan13) && !empty($kesimpulan13['keterangan'])): ?>
                        ...
                        <?php if (($userInGroupPE ?? false) || ($userInGroupAdmin ?? false)): ?>
                            <div class="collapse" id="editKesimpulan13">
                                <hr>
                                <form action="<?= base_url('Likuiditasinheren/simpanKesimpulan13') ?>" method="post">
                                    <?= csrf_field() ?>
            ...
            <div class="d-flex justify-content-center gap-2">
                <a href="<?= base_url('Likuiditasinheren/exporttxtrisikolikuiditas'); ?>"
                        ...
                        <div class="card shadow-sm approvaldir-card" style="height: 120px;">
                            <div class="card-body approvaldir-card-body">
                                ...
                                <div class="approval-buttons-container">
                                    <a href="<?= base_url('Likuiditasinheren/approveSemua') ?>"
                                    ...
                                    <a href="<?= base_url('Likuiditasinheren/unapproveSemua') ?>"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('Likuiditasinheren/ubah'); ?>" method="post">
                    <?= csrf_field() ?>
...
<div class="modal fade" id="modalUbahkesimpulan">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('Likuiditasinheren/ubahkesimpulan'); ?>" method="post">
                    <?= csrf_field() ?>
...
<div class="modal fade" id="modaltambahNilai">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?= base_url('Likuiditasinheren/tambahNilai'); ?>" method="post">
                <?= csrf_field() ?>
                <div class="modal-header">
...
<div class="modal fade" id="modaltambahKomentar">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?= base_url('Likuiditasinheren/tambahKomentar'); ?>" method="post">
                <?= csrf_field() ?>
                <div class="modal-header">
...
<script>
    ...
    function calculateRiskKalkulator() {
        ...
        const kas = getInputValue('kas');
        const girobanklain = getInputValue('girobanklain');
        const tabunganbanklain = getInputValue('tabunganbanklain');
        const kewajibansegera = getInputValue('kewajibansegera');
        const tabungandpk = getInputValue('tabungandpk');
        const depositodpk = getInputValue('depositodpk');
        const tabunganabp = getInputValue('tabunganabp');
        const depositoabp = getInputValue('depositoabp');
        const pinjamanditerima = getInputValue('pinjamanditerima');
        const kreditkyd = getInputValue('kreditkyd');
        const penabung25deposan = getInputValue('penabung25deposan');
        const totalpendanaan = getInputValue('totalpendanaan');
        const dpkdiataslps = getInputValue('dpkdiataslps');
        const pinjamananmungkinditarik = getInputValue('pinjamananmungkinditarik');

        const asetlikuid = kas + girobanklain + tabunganbanklain;
        const kewajibanlancar = kewajibansegera + tabungandpk + depositodpk + tabunganabp + depositoabp + pinjamanditerima;
        const totaldpk = tabungandpk + depositodpk;
        const transaksibpr = tabunganabp + depositoabp;
        const pendanaannoninti = dpkdiataslps + transaksibpr + pinjamananmungkinditarik;

        const rasioasetlikuidtotalaset = totalaset > 0 ? (asetlikuid / totalaset) * 100 : 0;
        const rasioasetlikuidkewajiban = kewajibanlancar > 0 ? (asetlikuid / kewajibanlancar) * 100 : 0;
        // const rasiokreditterhadapdpk = totaldpk > 0 ? (kreditkyd / totaldpk) * 100 : 0;
        const rasiokreditterhadapdpk = kreditkyd > 0 ? (totaldpk / kreditkyd) * 100 : 0;
        const rasio25deposan = totaldpk > 0 ? (penabung25deposan / totaldpk) * 100 : 0;
        const rasiononinti = totalpendanaan > 0 ? (pendanaannoninti / totalpendanaan) * 100 : 0;

        // ✅ PERBAIKAN: Simpan LANGSUNG ke _submit (konsisten dengan insertRasioToKertasKerja)
        setHiddenValue('asetlikuid_hidden', asetlikuid);
        setHiddenValue('kewajibanlancar_hidden', kewajibanlancar);
        setHiddenValue('totaldpk_hidden', totaldpk);
        setHiddenValue('transaksibpr_hidden', transaksibpr);
        setHiddenValue('pendanaannoninti_hidden', pendanaannoninti);

        // ✅ Yang dipakai untuk insert - simpan ke _submit
        setHiddenValue('rasioasetlikuidtotalaset_submit', rasioasetlikuidtotalaset);
        setHiddenValue('rasioasetlikuidkewajiban_submit', rasioasetlikuidkewajiban);
        setHiddenValue('rasiokreditterhadapdpk_submit', rasiokreditterhadapdpk);
        setHiddenValue('rasio25deposan_submit', rasio25deposan);
        setHiddenValue('rasiononinti_submit', rasiononinti);

        // Tampilkan tombol insert
        const btnInsertRasio = document.getElementById('btnInsertRasio');
        if (btnInsertRasio && (kas > 0 || girobanklain > 0 || tabunganbanklain > 0)) {
            btnInsertRasio.style.display = 'block';
        }

        // Tampilkan detail result
        const detailResult = document.getElementById('detailResult');
        if (detailResult) detailResult.style.display = 'block';

        // Update display
        updateDisplay('asetlikuid', formatCurrency(asetlikuid));
        updateDisplay('kewajibanlancar', formatCurrency(kewajibanlancar));
        updateDisplay('totaldpk_display', formatCurrency(totaldpk));
        updateDisplay('transaksibpr', formatCurrency(transaksibpr));
        updateDisplay('pendanaannoninti', formatCurrency(pendanaannoninti));
        updateDisplay('rasioasetlikuidtotalaset', rasioasetlikuidtotalaset.toFixed(2) + '%');
        updateDisplay('rasioasetlikuidkewajiban', rasioasetlikuidkewajiban.toFixed(2) + '%');
        updateDisplay('rasiokreditterhadapdpk', rasiokreditterhadapdpk.toFixed(2) + '%');
        updateDisplay('rasio25deposan', rasio25deposan.toFixed(2) + '%');
        updateDisplay('rasiononinti', rasiononinti.toFixed(2) + '%');
    ...
    function insertRasioToKertasKerja() {
        ...
        const rasioasetlikuidtotalaset = $('#rasioasetlikuidtotalaset_submit').val();
        const rasioasetlikuidkewajiban = $('#rasioasetlikuidkewajiban_submit').val();
        const rasiokreditterhadapdpk = $('#rasiokreditterhadapdpk_submit').val();
        const rasio25deposan = $('#rasio25deposan_submit').val();
        const rasiononinti = $('#rasiononinti_submit').val();

        // Debug log
        console.log('=== DEBUG INSERT RASIO ===');
        console.log('rasioasetlikuidtotalaset:', rasioasetlikuidtotalaset);
        console.log('rasioasetlikuidkewajiban:', rasioasetlikuidkewajiban);
        console.log('rasiokreditterhadapdpk:', rasiokreditterhadapdpk);
        console.log('rasio25deposan:', rasio25deposan);
        console.log('rasiononinti:', rasiononinti);

        // Validasi
        if (!rasioasetlikuidtotalaset && !rasioasetlikuidkewajiban &&
            !rasiokreditterhadapdpk && !rasio25deposan && !rasiononinti) {
        ...
        $.ajax({
            url: BASE_URL + '/Likuiditasinheren/insertRasioToKertasKerja',
            method: 'POST',
            data: {
    ...
    function renderApprovalButton(faktorId, nilai) {
        <?php if (($userInGroupAdmin ?? false) || ($userInGroupDireksi ?? false) || ($userInGroupDireksi2 ?? false)): ?>
            ...
            return `
            <form action="${BASE_URL}/Likuiditasinheren/${action}" method="POST" style="display: inline-block;" class="ml-2 form-approval">
    ...
    function pollUnreadCounts() {
        $.ajax({
            url: `${BASE_URL}/Likuiditasinheren/getAllUnreadCounts`,
            method: 'GET',
            dataType: 'json',
    ...
    function refreshFactorsData() {
        $.ajax({
            url: BASE_URL + '/Likuiditasinheren/getFactorsData',
            method: 'GET',
            success: function (response) {
                if (response.status === 'success') {
    ...
    function loadKomentarList(faktorId) {
        $.ajax({
            url: `${BASE_URL}/Likuiditasinheren/getKomentarByFaktorId/${faktorId}`,
            method: 'GET',
            data: {
    ...
    $(document).ready(function () {
        ...
        // Form submissions
        $('form[action*="Likuiditasinheren/ubah"]').on('submit', function (e) {
            e.preventDefault();
        ...
        $('form[action*="Likuiditasinheren/ubahkesimpulan"]').off('submit').on('submit', function (e) {
            e.preventDefault();
        ...
        $('form[action*="Likuiditasinheren/ubahketerangan"]').on('submit', function (e) {
            e.preventDefault();
        ...
        $('form[action*="Likuiditasinheren/tambahNilai"]').on('submit', function (e) {
            e.preventDefault();
        ...
        $('form[action*="Likuiditasinheren/tambahKomentar"]').on('submit', function (e) {
            e.preventDefault();
        ...
        $(document).on('click', '.komentar-button', function () {
            ...
            // Mark as read
            $.ajax({
                url: `${BASE_URL}/Likuiditasinheren/markUserCommentsAsRead`,
                method: 'POST',
                data: {
                    faktor_id: faktorId,
                    kodebpr: GLOBAL_KODEBPR,
