...
<div class="container-fluid">
    ...
    <div class="card shadow">
        <div class="card-headertitle">
            ...
            <h1 class="h3 mb-4 text-gray-800 text-center"><?= $judul; ?><br>(E0303)</h1>
        </div>
        ...
        <div class="card-body">
                    ...
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <div class="btn-group" role="group" aria-label="Button group">
                            <button type="button" class="btn btn-primarys btn-sm" data-toggle="modal"
                                data-target="#modalTambahsahamdir"><i class="fa fa-plus"></i> Tambah Data </button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                    ...
                    <?php if (empty($shmdirdekomlain)) { ?>
                        <tr>
                            <td colspan="3" class="text-center">
                                <em>Tidak ada data Kepemilikan Saham Anggota Direksi pada Perusahaan Lain</em>
                            </td>
                        </tr>
                    <?php } else { ?>
                        <?php
                        $i = 1;
                        foreach ($shmdirdekomlain as $row):
                            if (
                                !empty($row['nama']) && !is_null($row['nama']) &&
                                !empty($row['nik']) && !is_null($row['nik']) &&
                                !empty($row['jenisperusahaan']) && !is_null($row['jenisperusahaan']) &&
                                !empty($row['perusahaan']) && !is_null($row['perusahaan']) &&
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($row['jenisperusahaan']); ?></td>
                                </tr>
                                <tr>
                                    ...
                                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                                        <td colspan="3">
                                            <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                                data-target="#modalUbahdir" id="btn-edit" data-id="<?= $row['id']; ?>"
                                                data-nama="<?= htmlspecialchars($row['nama']); ?>"
                                                data-nik="<?= htmlspecialchars($row['nik']); ?>"
                                                data-jenisperusahaan="<?= htmlspecialchars($row['jenisperusahaan']); ?>"
                                                data-kodebank="<?= htmlspecialchars($row['kodebank'] ?? ''); ?>"
                                                data-perusahaan="<?= htmlspecialchars($row['perusahaan']); ?>"
            ...
            <table class="table table-info table-hover">
                <thead class="thead-primary">
                    <tr>
                        <th>Kepemilikan Saham Dewan Komisaris pada Perusahaan Lain</th>
                    </tr>
                </thead>
            </table>
            <table class="table table-bordered table-hover">
                <tbody>
                    <?php if (empty($shmdirdekomlain)) { ?>
                        <tr>
                            <td colspan="3" class="text-center">
                                <em>Tidak ada data Kepemilikan Saham Dewan Komisaris pada Perusahaan Lain</em>
                            </td>
                        </tr>
                    <?php } else { ?>
                        <?php
                            ...
                            if (
                                ...
                                <tr>
                                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                                        <td colspan="3">
                                            <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                                data-target="#modalUbahdekom" id="btn-edit" data-id="<?= $row['id']; ?>"
                                                data-nama="<?= htmlspecialchars($row['nama']); ?>"
                                                data-nik="<?= htmlspecialchars($row['nik']); ?>"
                                                data-jenisperusahaan="<?= htmlspecialchars($row['jenisperusahaan'] ?? ''); ?>"
                                                data-kodebank="<?= htmlspecialchars($row['kodebank']); ?>"
                    ...
                    <?php if (empty($penjelastindak)) { ?>
                        <tr>
                            <th class="table-info" style="width: 30%; color: black;">Penjelasan Lebih Lanjut Kepemilikan
                                Saham Anggota Direksi dan Anggota Dewan Komisaris pada BPR (Opsional) :
                            </th>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-center">
                        ...
                        <?php foreach ($penjelastindak as $row): ?>
                            <tr>
                                <th colspan="2" class="table-info" style="width: 30%; color: black;">Penjelasan Lebih Lanjut
                                    Kepemilikan Saham Anggota Direksi dan Anggota Dewan Komisaris pada BPR (Opsional) :</th>
                            </tr>
                            <tr>
                                <td colspan="2">
                        ...
                        <div class="card shadow-sm approval-card">
                            <div class="card-body approval-card-body">
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Shmdirdekomlain/approveSemuaKom') ?>"
                                    ...
                                    <a href="<?= base_url('Shmdirdekomlain/unapproveSemuaKom') ?>"
                    ...
                    <div class="col-md-3">
                        <div class="card shadow-sm approvaldir-card">
                            <div
                                ...
                                <div class="approval-buttons-container"> <a
                                        href="<?= base_url('Shmdirdekomlain/approveSemuaDirut') ?>"
                                    ...
                                    <a href="<?= base_url('Shmdirdekomlain/unapproveSemuaDirut') ?>"
            ...
            <div class="d-flex justify-content-center mb-5">
                <div class="d-flex justify-content-center gap-2 mb-5">
                    <?php if ($userInGroupPE || $userInGroupAdmin): ?>
                        <a href="<?= base_url('Shmdirdekomlain/exporttxtshmdirdekomlain'); ?>"
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group me-2" role="group" aria-label="First group">
                                <button type="button" class="btn btn-outline-primary btn-sm"
                                    onclick="window.location.href='<?= base_url('Shmusahadirdekom') ?>'">
                                    << </button>
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                        ...
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="window.location.href='<?= base_url('Keuangandirdekompshm') ?>'">>></button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?= base_url('shmdirdekomlain/ubahdir'); ?>" method="post">
                        ...
                        <div class="mb-3">
                            <label for="nama" class="form-label">Nama Anggota Direksi:</label>
                        ...
                        <div class="mb-3">
                            ...
                            <small id="nikdekomCounterUbah" class="form-text text-muted">0/16 digit</small>
                        </div>
                        <div class="mb-3">
                            <label for="jenisperusahaan" class="form-label">Jenis Perusahaan:</label>
                            <select name="jenisperusahaan" id="jenisperusahaan" class="form-control" required
                                onchange="toggleubahKodeDir()">
                                <option value="Perusahaan Lain" <?= (isset($row['jenisperusahaan']) && $row['jenisperusahaan'] == 'Perusahaan Lain') ? 'selected' : ''; ?>>Perusahaan Lain
                                </option>
                                <option value="Bank Lain" <?= (isset($row['jenisperusahaan']) && $row['jenisperusahaan'] == 'Bank Lain') ? 'selected' : ''; ?>>Bank Lain</option>
                            </select>
                        </div>
                        <div class="mb-3" id="kodebank-group" style="display: none;">
                            <label for="kodebank" class="form-label">Kode Bank:</label>
                            <input class="form-control" type="text" name="kodebank" id="kodebank"
                                value="<?= isset($row['kodebank']) ? esc($row['kodebank']) : ''; ?>" maxlength="16">
                        </div>
                        <div class="mb-3">
                            <label for="perusahaan" class="form-label">Nama Perusahaan / Bank Lain:</label>
                            <input class="form-control" type="text" name="perusahaan" id="perusahaan"
                                value="<?= isset($row['perusahaan']) ? esc($row['perusahaan']) : ''; ?>" maxlength="16"
                                required>
                        </div>
                        <div class="mb-3">
...
<?php if (!empty($shmdirdekomlain)): ?>
    <div class="modal fade" id="modalUbahdekom">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ubah Kepemilikan Saham Anggota Direksi dan Anggota Dewan Komisaris pada BPR</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="<?= base_url('Shmdirdekomlain/ubahdekom'); ?>" method="post">
                        <input type="hidden" name="id" id="id-shmdirdekomlain"
                            value="<?= isset($row['id']) ? esc($row['id']) : ''; ?>">
                        <div class="mb-3">
                            <label for="nama" class="form-label">Input Nama Dewan Komisaris:</label>
                            <input class="form-control" type="text" name="nama" id="nama"
                                value="<?= isset($row['nama']) ? esc($row['nama']) : ''; ?>" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="perusahaan" id="perusahaan"
                                placeholder="<?= isset($row['perusahaan']) ? esc($row['perusahaan']) : ''; ?>"
                                required></input>
                        </div>
                        <div class="form-group">
                            <label for="persensaham" class="form-label">Input Persentase Kepemilikan (%) Tahun
                                Sebelumnya: </label>
                            <input class="form-control" type="text" name="persensaham" id="persensaham"
                                placeholder="<?= isset($row['persensaham']) ? esc($row['persensaham']) : ''; ?>"
                                required></input>
                            <small id="persensaham" class="form-text text-muted">Hanya angka saja. Jika ada koma
                                gunakan titik (contoh: 23.45)</small>
                        </div>
...
<div class="modal fade" id="modalTambahketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('shmdirdekomlain/tambahketerangan'); ?>" method="post">
...
<div class="modal fade" id="modaleditketerangan">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('shmdirdekomlain/editketerangan'); ?>" method="post">
...
<div class="modal fade" id="modalTambahsahamdir">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Kepemilikan Saham Anggota Direksi pada Perusahaan Lain</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('Shmdirdekomlain/tambahsahamdir'); ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenisperusahaan" id="jenisperusahaan" class="form-control" required
                            onchange="toggleKodeBank()">
                            <option value="" readonly selected>Pilih Jenis Perusahaan</option>
                            <option value="Perusahaan Lain">Perusahaan Lain</option>
                            <option value="Bank Lain">Bank Lain</option>
                        </select>
                    </div>
                    <div class="form-group" id="kodebank-group" style="display: none;">
                        <label for="kodebank">Kode Bank:</label>
                        <input type="text" name="kodebank" id="kodebank" class="form-control">
                        <small id="kodebank" class="form-text text-muted">Note: Hanya angka saja.</small>
                    </div>
                    <div class="form-group">
                        <label for="perusahaan">Nama Perusahaan / Bank: </label>
                        <input type="text" name="perusahaan" id="perusahaan" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="persensaham">Persentase Kepemilikan (%): </label>
                        <input type="text" name="persensaham" id="persensaham" class="form-control" required>
                        <small id="persensaham" class="form-text text-muted">Hanya angka saja. Jika ada koma gunakan
                            titik (contoh: 23.45)</small>
                    </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
...
<div class="modal fade" id="modalTambahsahamdekom">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Kepemilikan Saham Anggota Dewan Komisaris pada Perusahaan Lain</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('Shmdirdekomlain/tambahsahamdekom'); ?>" method="post">
                    <div class="form-group">
                        <label for="nama">Nama Anggota Dewan Komisaris:</label>
                        <input type="text" name="nama" id="nama" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label for="nik" class="form-label">NIK:</label>
                        <input class="form-control" type="text" name="nik" id="nikdekomTambah" maxlength="16" required>
                        <small id="nikdekomCounterTambah" class="form-text text-muted">0/16 digit</small>
                    </div>
                    <div class="form-group">
                        <label for="jenisperusahaan">Jenis Perusahaan:</label>
                        <select name="jenisperusahaan" id="jenisperusahaan" class="form-control" required
...
<script>
    function toggleKodeBank() {
        var jenisPerusahaan = document.getElementById("jenisperusahaan").value;
        var kodebankGroup = document.getElementById("kodebank-group");

        if (jenisPerusahaan === "Bank Lain") {
    ...
    // Tambahkan onchange di select
    document.getElementById("jenisperusahaan").addEventListener("change", toggleKodeBank);
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    ...
                    window.location.href = "/shmdirdekomlain/setNullKolomTindak/" + id;
                }
            });
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnSetNulls.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                if (confirm("Apakah Anda yakin hendak menghapus data nilai dan keterangan ini?")) {
                    window.location.href = "/Shmdirdekomlain/setNullKolomPenjelaslanjut/" + id;
                }
            });
        });
    });
...
<script>
    ...
    function handleNikdekomCounter(inputId, counterId) {
        ...
        const nikdekomounter = document.getElementById(counterId);
    ...
    // Memanggil fungsi untuk form tambah
    handleNikCounter('nikdekomTambah', 'nikdekomCounterTambah');
    // Memanggil fungsi untuk form ubah
    handleNikCounter('nik', 'nikdekomCounterUbah');
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function fetchAndDisplayComments(Id, kodebpr, periodeId) {
            ...
            $.ajax({
                url: '<?= base_url('Shmdirdekomlain/getKomentarByFaktorId'); ?>/' + Id,
                method: 'GET',
                data: {
                    kodebpr: kodebpr,
        ...
        commentButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                $.ajax({
                    url: '<?= base_url('Shmdirdekomlain/markUserCommentsAsRead'); ?>',
                    method: 'POST',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function pollUnreadCounts() {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('Shmdirdekomlain/getUnreadCommentCountForFactor'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
        ...
        function refreshBadgeForAllUsers(exceptUserId) {
            ...
            commentButtons.forEach(button => {
                ...
                $.ajax({
                    url: '<?= base_url('Shmdirdekomlain/getUnreadCommentCountForAllUsers'); ?>',
                    method: 'GET',
                    data: {
                        id: Id,
                        kodebpr: kodebpr,
